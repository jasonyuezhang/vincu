...
Vincu surfaces terminal activity as a tab indicator (the same "running" dot used by agents).
...
OpenCode uses a server plugin instead of command hooks. The plugin listens to OpenCode bus events and emits these Vincu hook events:
...
removes Vincu's marker-matched hooks on disable. `vincu hooks` keeps working regardless — the gate
...
When enabled, Vincu installs provider hooks globally:

- Claude hooks are written to `~/.claude/settings.json` (or `CLAUDE_CONFIG_DIR/settings.json` when that override is set).
- Codex hooks are written to `~/.codex/hooks.json` (or `CODEX_HOME/hooks.json` when that override is set). Codex supports a native `commandWindows`, so each Vincu hook includes both POSIX and Windows commands. Non-managed Codex hooks are trust-gated by Codex; users may see Codex's hook review prompt before the hook runs.
- OpenCode gets a self-contained plugin at `$XDG_CONFIG_HOME/opencode/plugins/vincu-terminal-activity.js` (or `~/.config/opencode/plugins/vincu-terminal-activity.js` when XDG is unset; `OPENCODE_CONFIG_DIR` still wins when set).

Installation is marker-based/idempotent for config hooks and exact-file/idempotent for the OpenCode plugin. Vincu preserves user hooks, removes only its own marker-matched command hooks, and leaves hooks installed across daemon shutdown. Outside a Vincu terminal they are inert because the command or plugin is gated on `VINCU_TERMINAL_ID`.
...
```sh
[ -n "$VINCU_TERMINAL_ID" ] && "${VINCU_HOOK_CLI:-vincu}" hooks claude <event>
```

Codex also receives the Windows equivalent:

```bat
if defined VINCU_TERMINAL_ID (if defined VINCU_HOOK_CLI ("%VINCU_HOOK_CLI%" hooks codex <event>) else (vincu hooks codex <event>))
```

The daemon resolves the current CLI through `VINCU_CLI` when its launcher supplies one, or through the npm package shim for standalone installs. Terminal setup exposes that resolved executable to hooks as `VINCU_HOOK_CLI`; desktop and other daemon launchers do not know about the hook-specific variable. The generated command falls back to bare `vincu` if the hook env is missing and no-ops outside Vincu terminals because the `VINCU_TERMINAL_ID` gate remains first. Vincu also prepends the resolved CLI directory to each terminal `PATH` as a secondary fallback. All other behavior lives in `vincu hooks`: read the env, map the event, POST activity, and no-op/fail-open when anything is missing or unavailable.
