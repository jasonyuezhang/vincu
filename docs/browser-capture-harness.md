...
```bash
npm run capture-harness --workspace=@getvincu/desktop
```
...
VINCU_CAPTURE_HARNESS_GROUP=automation npm run capture-harness --workspace=@getvincu/desktop
```

Run the shared browser profile fixture with:

```bash
VINCU_CAPTURE_HARNESS_GROUP=browser-profile npm run capture-harness --workspace=@getvincu/desktop
```
