...
do not use `createVincuDaemon` as a product launch path.
...
const vincuHomeRoot = await mkdtemp(path.join(os.tmpdir(), "vincu-test-"));
...
For simpler cases, `createTestVincuDaemon` + `DaemonClient` handles temp dirs and port selection:
...
The test helper does **not** expose `providerOverrides`. In test harnesses, use `createVincuDaemon` directly when you need it (see quick start above).
