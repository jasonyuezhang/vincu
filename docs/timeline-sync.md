...
Provider message IDs are not guaranteed for every displayed item. Vincu-generated system errors are one example. Rendered item indices are not durable either because pagination and projection can merge source rows.
