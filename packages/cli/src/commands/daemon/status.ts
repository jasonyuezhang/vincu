...
import { getOrCreateServerId, findExecutable, execCommand } from "@getvincu/server";
...
function describeDaemonAuthProbeFailure(host: string, failure: DaemonAuthProbeFailure): string {
  if (failure === "auth_required") {
    return `Daemon is reachable at ${host} but requires a password. Set VINCU_PASSWORD and retry.`;
  }
  return `Daemon is reachable at ${host} but the supplied password was rejected. Check VINCU_PASSWORD and retry.`;
}
