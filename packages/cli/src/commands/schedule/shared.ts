...
function resolveScheduleTarget(args: {
  ...
  if (targetValue === "self") {
    // COMPAT(scheduleSelfTarget): heartbeat creation moved to `vincu heartbeat create`.
    // Added in v0.2.0; remove after 2027-01-17.
    const currentAgentId = process.env.VINCU_AGENT_ID?.trim();
    if (!currentAgentId) {
      throw {
        code: "INVALID_TARGET",
        message: "--target self requires running inside a Vincu agent",
      } satisfies CommandError;
    }
