...
export function resolveVincuHomePath(): string {
  return process.env.VINCU_HOME ?? join(homedir(), ".vincu");
}

export function resolveVincuWorktreesDir(): string {
  return join(resolveVincuHomePath(), "worktrees");
}

function isAgentInManagedWorktree(agentCwd: string): boolean {
  const worktreesDir = resolveVincuWorktreesDir();
...
export async function runLsCommand(
  ...
  try {
    ...
    const response = await client.getVincuWorktreeList({});
