...
function createFakeDaemonClient(
  overrides: Partial<
    Pick<DaemonClient, "getVincuWorktreeList" | "archiveVincuWorktree" | "close">
  > = {},
): DaemonClient {
  return {
    getVincuWorktreeList: async () => ({
      worktrees: [],
      error: null,
      requestId: "req-list",
    }),
    archiveVincuWorktree: async () => ({
      success: true,
      removedAgents: [],
      error: null,
...
describe("runArchiveCommand", () => {
  it("sends scope worktree when archiving by worktree path", async () => {
    const worktreePath = "/tmp/vincu-home/worktrees/repo/feature";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveVincuWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
      ...
      archiveVincuWorktree: async (input) => {
  ...
  it("archives by matching branch name when no directory name matches", async () => {
    const worktreePath = "/tmp/vincu-home/worktrees/repo/feature-branch";
    const archiveCalls: Array<{
      input: Parameters<DaemonClient["archiveVincuWorktree"]>[0];
    }> = [];
    const fakeClient = createFakeDaemonClient({
