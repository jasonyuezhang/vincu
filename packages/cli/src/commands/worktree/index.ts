...
export function createWorktreeCommand(): Command {
  const worktree = new Command("worktree").description("Manage Vincu-managed git worktrees");

  addJsonAndDaemonHostOptions(
    worktree.command("ls").description("List Vincu-managed git worktrees"),
  ).action(withOutput(runLsCommand));

  addJsonAndDaemonHostOptions(
    worktree
      .command("create")
      .description("Create a Vincu-managed git worktree")
