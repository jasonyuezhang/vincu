...
import type { AgentTimelineItem } from "@getvincu/protocol/agent-types";
import type { AgentStreamMessage } from "@getvincu/protocol/messages";
import { curateAgentActivity } from "@getvincu/server";
...
export async function runLogsCommand(
  ...
  if (!id) {
    ...
    console.error("Usage: vincu agent logs <id>");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error("Start the daemon with: vincu daemon start");
    process.exit(1);
  }
  ...
  try {
    ...
    if (!fetchResult) {
      ...
      console.error("Use `vincu ls` to list available agents");
      await client.close();
      process.exit(1);
    }
