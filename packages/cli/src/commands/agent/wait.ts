...
function buildWaitResult(args: {
  ...
  if (state.status === "timeout") {
    const timeoutMessage = timeoutLabel
      ? `Agent did not finish within ${timeoutLabel}. Run \`vincu wait ${resolvedAgentId}\` again to keep waiting.`
      : `Agent wait timed out. Run \`vincu wait ${resolvedAgentId}\` again to keep waiting.`;
    return {
...
export async function runWaitCommand(
  ...
  if (!agentIdArg || agentIdArg.trim().length === 0) {
    throw {
      ...
      details: "Usage: vincu agent wait <id>",
    } satisfies CommandError;
  }
