 ...
 * - `vincu agent ls`    → active non-archived agents
 * - `vincu agent ls -g` → global non-archived agents
 * - `vincu agent ls -a` → active agents, including archived
 * - `vincu agent ls -ag` → global agents, including archived
 */
export async function runLsCommand(
  ...
  try {
    ...
    const error: CommandError = {
      ...
      details:
        "Start the daemon with: vincu daemon start\nFor a remote daemon, pass --host <host:port> or set VINCU_HOST.",
    };
    throw error;
  }
