...
describe("managed agent caller context", () => {
  it("propagates a trimmed VINCU_AGENT_ID", () => {
    expect(resolveRunCallerAgentId({ VINCU_AGENT_ID: "  parent-agent  " })).toBe("parent-agent");
  });

  it("omits blank caller ids", () => {
    expect(resolveRunCallerAgentId({ VINCU_AGENT_ID: "   " })).toBeUndefined();
  });
});
...
describe("runRunCommand option validation", () => {
  const originalWorkspaceId = process.env.VINCU_WORKSPACE_ID;

  beforeEach(() => {
    delete process.env.VINCU_WORKSPACE_ID;
  });
  ...
  afterEach(() => {
    if (originalWorkspaceId === undefined) {
      ...
      process.env.VINCU_WORKSPACE_ID = originalWorkspaceId;
    }
  });
