...
import { spawnProcess } from "@getvincu/server";
import { buildAgentDeepLink, type AgentDeepLinkTarget } from "@getvincu/protocol/agent-deep-link";

function findDesktopApp(): string | null {
  if (process.platform === "darwin") {
    const candidates = [
      "/Applications/Vincu.app",
      path.join(homedir(), "Applications", "Vincu.app"),
    ];
  ...
  if (process.platform === "linux") {
    const candidates = [
      "/usr/bin/Vincu",
      "/opt/Vincu/Vincu",
      path.join(homedir(), "Applications", "Vincu.AppImage"),
    ];
  ...
  if (process.platform === "win32") {
    ...
    const candidate = path.join(localAppData, "Programs", "Vincu", "Vincu.exe");
...
function launchDesktop(args: string[]): void {
  if (process.env.VINCU_DESKTOP_CLI === "1") {
    throw new Error("Cannot open Vincu Desktop while running in desktop CLI passthrough mode.");
  }

  const desktopApp = findDesktopApp();
  if (!desktopApp) {
    throw new Error(
      "Vincu desktop app not found. Install it from https://github.com/getvincu/vincu/releases",
    );
  }
