...
describe("runCli", () => {
  ...
  it("routes explicit root relay flags to onboard", () => {
    ...
    ).toEqual(["node", "vincu", "onboard", "--relay"]);
    expect(
      createCliParseArgv({
        argv: ["--no-relay"],
        cwd: process.cwd(),
    ...
    ).toEqual(["node", "vincu", "onboard", "--no-relay"]);
  });
  ...
  it("preserves known CLI command argv", () => {
    ...
    ).toEqual(["node", "vincu", "daemon", "set-password"]);
  });
  ...
  it("preserves the hooks command argv", () => {
    ...
    ).toEqual(["node", "vincu", "hooks", "claude", "UserPromptSubmit"]);
  });

  it("classifies existing unknown directories as open-project invocations", () => {
    const root = mkdtempSync(path.join(tmpdir(), "vincu-cli-run-"));
