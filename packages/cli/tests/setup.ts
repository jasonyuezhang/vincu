/**
 * Test setup utilities for Vincu CLI E2E tests
 *
...
export interface TestContext {
  ...
  /** Run a vincu CLI command against the test daemon */
  vincu: (args: string[]) => ProcessPromise;
...
export async function createTempDirs(): Promise<{ vincuHome: string; workDir: string }> {
  ...
  const workDir = await mkdtemp(join(tmpdir(), "vincu-test-work-"));
  return { vincuHome, workDir };
}

/**
 * Wait for daemon to be ready by testing WebSocket connection
 * Uses `vincu agent ls` which connects via WebSocket
 */
async function probeDaemon(port: number): Promise<boolean> {
  try {
    const result = await $`VINCU_HOST=localhost:${port} vincu agent ls`.nothrow();
 ...
 */
export async function startDaemon(port: number, vincuHome: string): Promise<ProcessPromise> {
  $.verbose = false;
  const daemon =
    $`VINCU_HOME=${vincuHome} VINCU_LISTEN=127.0.0.1:${port} VINCU_RELAY_ENABLED=false VINCU_LOCAL_SPEECH_AUTO_DOWNLOAD=${TEST_ENV_DEFAULTS.VINCU_LOCAL_SPEECH_AUTO_DOWNLOAD} VINCU_DICTATION_ENABLED=${TEST_ENV_DEFAULTS.VINCU_DICTATION_ENABLED} VINCU_VOICE_MODE_ENABLED=${TEST_ENV_DEFAULTS.VINCU_VOICE_MODE_ENABLED} CI=true vincu daemon start --foreground`.nothrow();
  return daemon;
}
...
export async function createTestContext(): Promise<TestContext> {
  ...
  const { vincuHome, workDir } = await createTempDirs();

  // Helper to run CLI commands against test daemon
  const vincu = (args: string[]): ProcessPromise => {
    $.verbose = false;
    return $`VINCU_HOST=localhost:${port} VINCU_LOCAL_SPEECH_AUTO_DOWNLOAD=${TEST_ENV_DEFAULTS.VINCU_LOCAL_SPEECH_AUTO_DOWNLOAD} VINCU_DICTATION_ENABLED=${TEST_ENV_DEFAULTS.VINCU_DICTATION_ENABLED} VINCU_VOICE_MODE_ENABLED=${TEST_ENV_DEFAULTS.VINCU_VOICE_MODE_ENABLED} vincu ${args}`.nothrow();
  };
...
export async function createTestContextWithDaemon(): Promise<TestContext> {
  ...
  ctx.daemon = await startDaemon(ctx.port, ctx.vincuHome);
