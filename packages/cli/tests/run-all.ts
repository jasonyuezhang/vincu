...
/**
 * Test runner for Vincu CLI E2E tests
 *
 ...
 * its own daemon (ephemeral port + tmp VINCU_HOME), so parallelism is safe.
 */
...
// packages live in the root. Prepend it so `npx vincu` resolves locally.
...
const concurrencyEnv = process.env.VINCU_CLI_TEST_CONCURRENCY;
...
const shardTotal = parsePositiveInt(process.env.VINCU_CLI_TEST_SHARD_TOTAL, 1);
const shardIndexRaw = parsePositiveInt(process.env.VINCU_CLI_TEST_SHARD, 1);
if (shardIndexRaw < 1 || shardIndexRaw > shardTotal) {
  throw new Error(
    `VINCU_CLI_TEST_SHARD=${shardIndexRaw} out of range for SHARD_TOTAL=${shardTotal}`,
  );
}
...
async function writeJsonSummary({
  ...
  await writeFile(
    ...
    JSON.stringify(
      {
        suite: "cli-local",
        command: "npm run test:local --workspace=@getvincu/cli",
        counts: {
          passed,
          failed,
          skipped: 0,
...
console.log("🧪 Vincu CLI E2E Test Runner\n");
console.log("=".repeat(50));
...
async function runSingleTest(testFile: string): Promise<TestOutcome> {
  ...
  const npmCache = await mkdtemp(join(tmpdir(), "vincu-cli-test-npm-cache-"));
    ...
    return await new Promise<TestOutcome>((resolve) => {
      const proc = spawn("npx", ["tsx", testPath], {
        env: {
          ...
          VINCU_LOCAL_SPEECH_AUTO_DOWNLOAD: testEnvDefaults.VINCU_LOCAL_SPEECH_AUTO_DOWNLOAD,
          VINCU_DICTATION_ENABLED: testEnvDefaults.VINCU_DICTATION_ENABLED,
          VINCU_VOICE_MODE_ENABLED: testEnvDefaults.VINCU_VOICE_MODE_ENABLED,
        },
