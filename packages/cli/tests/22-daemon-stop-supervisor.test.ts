...
/**
 * Regression: `vincu daemon stop` must stop supervised dev daemons
...
async function readPidLockState(vincuHome: string): Promise<PidLockState> {
...
async function readCapturedSupervisorLogs(vincuHome: string, recentLogs: string): Promise<string> {
  const durableLogs = await readFile(join(vincuHome, "daemon.log"), "utf8").catch(() => "");
  return `${recentLogs}\n${durableLogs}`;
}
...
const vincuHome = await mkdtemp(join(tmpdir(), "vincu-stop-supervisor-"));
...
try {
  console.log("Test 1: start supervisor-entrypoint in dev mode with isolated VINCU_HOME");
  ...
  const statusBeforeStop = await readDaemonStatus(vincuHome);
  ...
  const pidLockBeforeStop = await readPidLockState(vincuHome);
  ...
  console.log("Test 2: `vincu daemon stop` should stop without respawn");
  const stopResult =
  ...
  const pidAfterStop = await readPidLockState(vincuHome);
  ...
  const statusAfterStop = await readDaemonStatus(vincuHome);
  assert.strictEqual(
    statusAfterStop.localDaemon,
    "stopped",
    "daemon should remain stopped after stop command",
  );
  const capturedSupervisorLogs = await readCapturedSupervisorLogs(vincuHome, recentSupervisorLogs);
  assert(
