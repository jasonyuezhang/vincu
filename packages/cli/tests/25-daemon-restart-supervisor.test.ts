...
const vincuHome = await mkdtemp(join(tmpdir(), "vincu-restart-supervisor-"));
...
try {
  ...
  const statusBeforeRestart = await readDaemonStatus(vincuHome);
  ...
  const statusAfterRestart = await readDaemonStatus(vincuHome);
  assert.strictEqual(
