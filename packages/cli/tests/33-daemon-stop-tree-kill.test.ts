...
const vincuHome = await mkdtemp(join(tmpdir(), "vincu-stop-tree-kill-"));
const childPidPath = join(vincuHome, "descendant.pid");
...
try {
  ...
  const stopResult =
    await $`VINCU_HOME=${vincuHome} VINCU_LOCAL_SPEECH_AUTO_DOWNLOAD=${testEnv.VINCU_LOCAL_SPEECH_AUTO_DOWNLOAD} VINCU_DICTATION_ENABLED=${testEnv.VINCU_DICTATION_ENABLED} VINCU_VOICE_MODE_ENABLED=${testEnv.VINCU_VOICE_MODE_ENABLED} npx vincu daemon stop --home ${vincuHome} --json --timeout 1 --force --kill-timeout 2`.nothrow();
