...
try {
...

  {
    ...
    const posted = await ctx.vincu(["chat", "post", "coord-room", "first message for @agent-1"], {
      env: { VINCU_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
    });
    ...
    const read = await ctx.vincu(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.vincu(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    // always wakes it up.
    const waitPromise = ctx.vincu(["chat", "wait", "coord-room", "--timeout", "30s"]);
    const waitSentinel = waitPromise.then(() => "settled" as const);
    let postedSecond = false;
    for (let attempt = 0; attempt < 60; attempt++) {
      const post = await ctx.vincu(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.vincu(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.vincu(["chat", "delete", "coord-room"]);
