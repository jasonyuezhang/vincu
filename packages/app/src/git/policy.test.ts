...
import { CheckoutPrStatusSchema } from "@getvincu/protocol/messages";
...
describe("git-actions-policy", () => {
  ...
  it("keeps push available for a no-upstream Vincu worktree with local commits", () => {
    const actions = buildGitActions(
      createInput({
        hasRemote: true,
  ...
  it("offers archive workspace for Git checkouts and worktrees", () => {
    ...
    const worktree = buildGitActions(
      createInput({ hasUncommittedChanges: true, isVincuOwnedWorktree: true }),
    );
  ...
  it("still promotes archive as primary for an idle Vincu-owned worktree", () => {
    const actions = buildGitActions(createInput({ isVincuOwnedWorktree: true }));
