...
import type { CheckoutPrMergeMethod, PullRequestMergeable } from "@getvincu/protocol/messages";
...
function getPrimaryActionId(input: BuildGitActionsInput): GitActionId | null {
  ...
  // Only Vincu-owned worktrees get Archive as a fallback primary action.
  // Regular Git checkouts should not show the destructive archive CTA by default.
  if (input.isVincuOwnedWorktree) {
    return "archive-workspace";
  }
...
function hasPushableCommits(input: BuildGitActionsInput): boolean {
  ...
  // No-upstream Vincu worktrees are first-pushable: the daemon push sets upstream with `git push -u`.
  // Do not fold this into aheadOfOrigin; null also covers deleted/pruned upstream branches.
  return input.isVincuOwnedWorktree && input.aheadOfOrigin === null && input.aheadCount > 0;
}
