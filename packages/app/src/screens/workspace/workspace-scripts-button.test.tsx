...
const {
  ...
  const routePreferenceByServerId: Record<string, "public" | "vincu" | "direct"> = {};
  const routePreferenceListeners = new Set<() => void>();
  const setPreferredRoute = vi.fn((serverId: string, kind: "public" | "vincu" | "direct") => {
  ...
  it("uses service icon color for service health and running unknown status only", () => {
    current = renderScripts([
      ...
      script({
        scriptName: "worker",
        type: "service",
        hostname: "worker.vincu.localhost",
        lifecycle: "running",
        health: null,
        port: 5000,
      }),
      script({
        ...
        type: "service",
        hostname: "old-service.vincu.localhost",
        lifecycle: "stopped",
        exitCode: 1,
      }),
    ]);
