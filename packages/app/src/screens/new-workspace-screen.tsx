...
import { resolveTerminalProfiles } from "@getvincu/protocol/terminal-profiles";
...
import type { AgentProvider } from "@getvincu/protocol/agent-types";
...
async function createAndMergeWorkspace(input: {
  ...
  createInput: Parameters<
    NonNullable<ReturnType<typeof useHostRuntimeClient>>["createVincuWorktree"]
  >[0];
  mergeWorkspaces: (
    serverId: string,
  ...
  const payload = await input.client.createVincuWorktree(input.createInput);
