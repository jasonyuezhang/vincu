...
function makeSources(): DesktopDiagnosticSources {
  return {
    ...
    getDaemonLogs: async () => ({
      logPath: "/vincu/home/daemon.log",
      contents: "daemon line one\ndaemon line two",
    }),
    getAppLogs: async () => ({
      logPath: "/logs/Vincu/main.log",
...
describe("desktop diagnostic report", () => {
  ...
  test("includes the Electron main-process log after the daemon log", async () => {
    ...
    expect(report).toContain("  Log path: /vincu/home/daemon.log");
    expect(report).toContain("  App log path: /logs/Vincu/main.log");
