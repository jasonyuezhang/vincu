...
function getBrowserTerminal(): BrowserTerminal {
  const terminal = window.__vincuTerminal as BrowserTerminal | undefined;
  if (!terminal) {
...
describe("terminal emulator runtime in a real browser", () => {
  it("passes configured scrollback to xterm", async () => {
    ...
    await waitFor({
      predicate: () => window.__vincuTerminal !== undefined,
    });

    expect(window.__vincuTerminal?.options.scrollback).toBe(42_000);
  });
  ...
  it("updates scrollback on the mounted xterm", async () => {
    ...
    const terminal = window.__vincuTerminal;

    mounted.runtime.setScrollback({ lines: 42_000 });

    expect(window.__vincuTerminal).toBe(terminal);
    expect(window.__vincuTerminal?.options.scrollback).toBe(42_000);
  });
