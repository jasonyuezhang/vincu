...
} from "@getvincu/protocol/terminal-input-mode";
import {
...
export class TerminalEmulatorRuntime {
  ...
  mount(input: TerminalEmulatorRuntimeMountInput): void {
    ...
    window.__vincuTerminal = terminal;
  ...
  unmount(): void {
    ...
    this.cleanup?.();
    this.cleanup = null;
    if (window.__vincuTerminal === this.terminal) {
      window.__vincuTerminal = undefined;
    }
    this.terminal = null;
    this.fitAddon = null;
