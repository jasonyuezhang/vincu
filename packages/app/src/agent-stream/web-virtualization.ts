...
export function getWebPartialVirtualizationThreshold(): number {
  const override = readPositiveIntegerOverride(
    (globalThis as BottomAnchorE2ETestGlobals).__VINCU_E2E_WEB_PARTIAL_VIRTUALIZATION_THRESHOLD,
  );
...
export function getWebMountedRecentStreamItems(): number {
  const override = readPositiveIntegerOverride(
    (globalThis as BottomAnchorE2ETestGlobals).__VINCU_E2E_WEB_MOUNTED_RECENT_STREAM_ITEMS,
  );
