...
const VINCU_WORKTREE_PATH_PATTERN = /(^|\/)\.vincu\/worktrees(\/|$)/;
...
export function collectAgentWorkingDirectorySuggestions(
  ...
  for (const source of sources) {
    ...
    if (isVincuOwnedWorktreePath(cwd)) {
      continue;
    }
...
function isVincuOwnedWorktreePath(cwd: string): boolean {
  return VINCU_WORKTREE_PATH_PATTERN.test(cwd.replace(/\\/g, "/"));
}
