import type {
  VincuConfigRaw,
  VincuMetadataGeneration,
  VincuMetadataGenerationEntry,
  VincuScriptEntryRaw,
...
export interface ProjectConfigDraft {
  ...
  metadataGenerationBase: VincuMetadataGeneration | undefined;
}
...
export function configToDraft(config: VincuConfigRaw | null | undefined): ProjectConfigDraft {
...
interface ApplyDraftInput {
  ...
  base: VincuConfigRaw | null | undefined;
}

export function applyDraftToConfig(input: ApplyDraftInput): VincuConfigRaw {
  ...
  const nextScripts: Record<string, VincuScriptEntryRaw> = {};
  for (const row of input.draft.scripts) {
    ...
    nextScripts[trimmedName] = nextEntry as VincuScriptEntryRaw;
  }
  ...
  return result as VincuConfigRaw;
}
