...
describe("resolveNewAgentWorkingDir", () => {
  ...
  it("falls back to repo root when checkout metadata is unavailable", () => {
    expect(resolveNewAgentWorkingDir("/repo/.vincu/worktrees/feature", null)).toBe("/repo");
  });

  it("supports windows-style vincu worktree paths without checkout metadata", () => {
    expect(resolveNewAgentWorkingDir("C:\\Users\\me\\repo\\.vincu\\worktrees\\feature", null)).toBe(
      "C:\\Users\\me\\repo",
    );
  });

  it("returns the main repo root for vincu-owned worktrees", () => {
    const checkout = {
      isVincuOwnedWorktree: true,
      worktreeRoot: "/repo/.vincu/worktrees/feature",
      mainRepoRoot: "/repo/main",
    } as unknown as CheckoutStatusPayload;

    expect(resolveNewAgentWorkingDir("/repo/.vincu/worktrees/feature", checkout)).toBe(
      "/repo/main",
    );
  });
});
