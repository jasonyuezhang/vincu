...
describe("groupAgents", () => {
  it("groups active agents by remote URL when available", () => {
    const agents = [
      makeAgent({ id: "a1", cwd: "/Users/me/dev/vincu" }),
      makeAgent({ id: "a2", cwd: "/Users/me/dev/vincu-fix/worktree" }),
    ];

    const { activeGroups } = groupAgents(agents, {
      getRemoteUrl: () => "git@github.com:getvincu/vincu.git",
    });
