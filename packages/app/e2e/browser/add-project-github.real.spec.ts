...
test.describe("Add Project GitHub flow", () => {
  ...
  test("searches the host's repositories and clones into a clearly shown final path", async ({
    ...
    const parentDirectory = await mkdtemp(path.join(tmpdir(), "vincu-e2e-github-clone-"));
    ...
    try {
      ...
      await addProjectFlowInput(page).fill("getvincu/vincu");
      await expect(addProjectFlow(page).getByText("getvincu/vincu", { exact: true })).toBeVisible({
        timeout: 30_000,
      });
