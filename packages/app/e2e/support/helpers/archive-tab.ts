...
export async function primeAdditionalPage(page: Page): Promise<void> {
  ...
  await page.addInitScript(
    ({ daemon: seededDaemon, preferences: seededPreferences, seedNonce: nonce }) => {
      ...
      localStorage.setItem("@vincu:daemon-registry", JSON.stringify([seededDaemon]));
      localStorage.removeItem("@vincu:settings");
      localStorage.setItem("@vincu:create-agent-preferences", JSON.stringify(seededPreferences));
    },
