...
export interface VincuHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveVincuHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkVincuHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<VincuHomeMetadataForkResult> {
  const sourceHome = resolveVincuHomePath(input.sourceHome);
  const targetHome = resolveVincuHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork Vincu metadata onto the same VINCU_HOME.");
  }
