...
import type { DaemonClient as InternalDaemonClient } from "@getvincu/client/internal/daemon-client";
...
export async function archiveWorkspaceFromDaemon(
  ...
  const payload = await client.archiveVincuWorktree({
...
export async function createWorktreeViaDaemon(
  ...
  const payload = await client.createVincuWorktree({
    cwd: input.cwd,
