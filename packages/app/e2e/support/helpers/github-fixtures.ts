...
async function seedPr(args: {
  ...
  git(["config", "user.email", "e2e@vincu.test"], localPath);
  git(["config", "user.name", "Vincu E2E"], localPath);
...
const TEMP_GITHUB_REPO_PREFIX = "vincutmp-";
...
export async function createTempGithubRepo(options: {
  ...
  git(["config", "user.email", "e2e@vincu.test"], basePath);
  git(["config", "user.name", "Vincu E2E"], basePath);
...
export async function cloneGithubRepoDefaultBranchOnly(
  ...
  git(["config", "user.email", "e2e@vincu.test"], clonePath);
  git(["config", "user.name", "Vincu E2E"], clonePath);
...
export async function createLocalGithubPrFixture(): Promise<LocalGhPrFixture> {
  const fixtureRoot = await mkdtemp(path.join("/tmp", "vincu-e2e-local-github-pr-"));
  ...
  const githubUrl = "https://github.com/vincu-e2e/local-fixture.git";
  ...
  git(["config", "user.email", "e2e@vincu.test"], checkoutPath);
  git(["config", "user.name", "Vincu E2E"], checkoutPath);
