...
export const createTempGitRepo = async (
  ...
  execSync('git config user.email "e2e@vincu.test"', { cwd: repoPath, stdio: "ignore" });
  execSync('git config user.name "Vincu E2E"', { cwd: repoPath, stdio: "ignore" });
  execSync("git config commit.gpgsign false", { cwd: repoPath, stdio: "ignore" });
  await writeFile(path.join(repoPath, "README.md"), "# Temp Repo\n");
  if (options?.vincuConfig) {
    await writeFile(
      path.join(repoPath, "vincu.json"),
      JSON.stringify(options.vincuConfig, null, 2),
    );
  }
  ...
  if (options?.vincuConfig) {
    execSync("git add vincu.json", { cwd: repoPath, stdio: "ignore" });
  }
  for (const file of options?.files ?? []) {
  ...
  for (const branch of branches) {
    ...
    const markerPath = `.vincu-e2e-${branch.replace(/[^a-zA-Z0-9._-]/g, "-")}.txt`;
 ...
 */
export async function createTempDirectory(prefix = "vincu-e2e-dir-"): Promise<TempDirectory> {
...
export async function trackForkUpstream(repoPath: string): Promise<string> {
  ...
  const upstreamClone = await mkdtemp(path.join(await resolveTempRoot(), "vincu-e2e-upstream-"));
  ...
  execSync('git config user.email "e2e@vincu.test"', { cwd: upstreamClone, stdio: "ignore" });
  execSync('git config user.name "Vincu E2E"', { cwd: upstreamClone, stdio: "ignore" });
...
export async function readWorktreeBaseMetadata(
  ...
  const metadata = JSON.parse(
    await readFile(path.join(gitDir, "vincu", "worktree.json"), "utf8"),
