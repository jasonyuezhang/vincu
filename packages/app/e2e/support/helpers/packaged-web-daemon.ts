...
export async function startPackagedWebDaemon(input: {
  ...
  const home = await mkdtemp(path.join(tmpdir(), "vincu-relay-deployment-e2e-"));
  const serverId = `relay-deployment-${Date.now().toString(36)}`;
  const vincu = path.resolve(__dirname, "../../../../../node_modules/.bin/vincu");
  const env: NodeJS.ProcessEnv = {
    ...
    VINCU_RELAY_ENDPOINT: input.relayEndpoint,
    VINCU_RELAY_PUBLIC_ENDPOINT: input.relayEndpoint,
    VINCU_RELAY_USE_TLS: "false",
    VINCU_RELAY_PUBLIC_USE_TLS: "false",
  };
  ...
  try {
    ...
    return {
      ...
      close: async () => {
        await execFileAsync(vincu, ["daemon", "stop", "--home", home], { env }).catch(
          () => undefined,
        );
    ...
    await execFileAsync(vincu, ["daemon", "stop", "--home", home], { env }).catch(() => undefined);
    await rm(home, { recursive: true, force: true });
    throw error;
  }
}
