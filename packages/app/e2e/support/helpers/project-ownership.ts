...
interface ProjectClient {
  ...
  close(): Promise<void>;
  createVincuWorktree?: (...args: unknown[]) => Promise<OwnedResourceResult>;
...
export function withProjectOwnership<Client extends object>(client: Client): Client {
  ...
  return new Proxy(client, {
    get(_target, property) {
      ...
      if (property === "createVincuWorktree" && target.createVincuWorktree) {
        const createVincuWorktree = target.createVincuWorktree.bind(target);
        return async (...args: unknown[]) => trackProject(await createVincuWorktree(...args));
      }
