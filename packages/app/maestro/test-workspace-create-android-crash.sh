...
#   VINCU_MAESTRO_APP_ID=sh.vincu.debug
#   VINCU_MAESTRO_DIRECT_ENDPOINT=127.0.0.1:6767
#   VINCU_MAESTRO_DAEMON_WS_URL=ws://127.0.0.1:6767/ws
#   VINCU_MAESTRO_PROJECT_PATH=/path/to/git/repo
set -euo pipefail
...
OUT_DIR="/tmp/vincu-workspace-create-android-$(date +%s)"
...
render_flow() {
  ...
  perl -0pe '
    ...
    s/\$\{VINCU_MAESTRO_PROJECT_NAME\}/$ENV{VINCU_MAESTRO_PROJECT_NAME}/g;
  ' "$source" > "$target"
}
...
if [ -z "${VINCU_MAESTRO_PROJECT_PATH:-}" ]; then
  PROJECT_PARENT="$(mktemp -d /tmp/vincu-maestro-project-XXXXXX)"
  PROJECT_BASENAME="aaa-workspace-create-android-$(basename "$PROJECT_PARENT")"
  export VINCU_MAESTRO_PROJECT_PATH="$PROJECT_PARENT/$PROJECT_BASENAME"
  mkdir -p "$VINCU_MAESTRO_PROJECT_PATH"
  git -C "$VINCU_MAESTRO_PROJECT_PATH" init >/dev/null
  git -C "$VINCU_MAESTRO_PROJECT_PATH" checkout -b main >/dev/null 2>&1 || true
  git -C "$VINCU_MAESTRO_PROJECT_PATH" config user.name "Vincu Maestro"
  git -C "$VINCU_MAESTRO_PROJECT_PATH" config user.email "maestro@getvincu.local"
  printf "# Workspace create Android repro\n" > "$VINCU_MAESTRO_PROJECT_PATH/README.md"
  git -C "$VINCU_MAESTRO_PROJECT_PATH" add README.md
  git -C "$VINCU_MAESTRO_PROJECT_PATH" commit -m "Initial commit" >/dev/null
else
  PROJECT_PARENT=""
fi

export VINCU_MAESTRO_PROJECT_NAME="${VINCU_MAESTRO_PROJECT_NAME:-$(basename "$VINCU_MAESTRO_PROJECT_PATH")}"

echo "=== Workspace Create Android Crash Harness ==="
echo "Output dir: $OUT_DIR"
echo "App id: $VINCU_MAESTRO_APP_ID"
echo "Android direct endpoint: $VINCU_MAESTRO_DIRECT_ENDPOINT"
echo "Daemon websocket: $VINCU_MAESTRO_DAEMON_WS_URL"
echo "Project: $VINCU_MAESTRO_PROJECT_PATH"
echo "Project name: $VINCU_MAESTRO_PROJECT_NAME"
...
const projectPath = process.env.VINCU_MAESTRO_PROJECT_PATH;
const daemonUrl = process.env.VINCU_MAESTRO_DAEMON_WS_URL;
