...
Start the `sh.vincu.debug` dev client against the intended Metro server first;
...
working with `${VINCU_MAESTRO_*}` placeholders.
...
The scripts assume a development build with package id `sh.vincu.debug`, an
