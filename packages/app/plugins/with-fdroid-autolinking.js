...
const FDROID_ABI_VERSION_CODE_BLOCK = `// Vincu F-Droid single-ABI version codes
def vincuAbiVersionCodes = [
    "armeabi-v7a": 1,
    "arm64-v8a": 2,
    "x86": 3,
    "x86_64": 4,
]
def vincuArchitectures = (findProperty("reactNativeArchitectures") ?: "")
    .toString()
    .split(",")
    .collect { it.trim() }
    .findAll { !it.isEmpty() }

if (vincuArchitectures.size() == 1) {
    def vincuAbi = vincuArchitectures[0]
    def vincuAbiVersionCode = vincuAbiVersionCodes[vincuAbi]
    if (vincuAbiVersionCode == null) {
        throw new GradleException("Unsupported Vincu Android ABI: " + vincuAbi)
    }
    android.defaultConfig.versionCode = android.defaultConfig.versionCode * 10 + vincuAbiVersionCode
}
`;
...
function configureFdroidAppBuildGradle(contents) {
  ...
  if (!configuredContents.includes("// Vincu F-Droid single-ABI version codes")) {
