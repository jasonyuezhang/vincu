...
description: Vincu is an open source Claude Desktop alternative for developers who want Linux, self-hosting, native mobile apps, and Claude Code alongside Codex, OpenCode, Copilot, and more.
nav: Claude Desktop
order: 55
---

# Vincu vs Claude Desktop
...
Vincu runs a daemon on your machine. Desktop, web, mobile, and CLI clients connect to it over a websocket. The daemon launches Claude Code and other providers as local processes, using your installed CLIs, credentials, MCP servers, skills, and project config.
...
```bash
vincu run --provider claude "implement OAuth"
vincu run --provider codex --worktree refactor-auth "refactor auth"
...
See also: [Vincu vs Codex App](/alternatives/codex-app), [Vincu vs OpenCode Desktop](/alternatives/opencode-desktop), [Vincu vs Conductor](/alternatives/conductor).
