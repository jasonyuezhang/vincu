...
const LINUX_APPIMAGE_ASSET_PATTERN =
  /^Vincu-(?:\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)-)?x86_64\.AppImage$/;
...
const GITHUB_RELEASES_URL = "https://api.github.com/repos/getvincu/vincu/releases?per_page=10";
...
function pickWindowsAssets(assets: GitHubAsset[]) {
  const x64Suffixed = assets.find((asset) => /Vincu-Setup-.*-x64\.exe$/.test(asset.name));
  const arm64 = assets.find((asset) => /Vincu-Setup-.*-arm64\.exe$/.test(asset.name));
  const legacy = assets.find(
    (asset) =>
      /Vincu-Setup-.*\.exe$/.test(asset.name) &&
...
export function getLatestAndroidVersionFromReleases(releases: GitHubRelease[]): string {
  const release = releases.find((candidate) => {
    ...
    return candidate.assets.some(
      (asset) => asset.name === `vincu-${candidate.tag_name}-android.apk`,
    );
  });
...
function isReleaseInfo(value: unknown): value is ReleaseInfo {
  ...
  return (
    ...
    (record.linuxAppImageAsset === "Vincu-x86_64.AppImage" ||
      new RegExp(`^Vincu-${record.version.replaceAll(".", "\\.")}-x86_64\\.AppImage$`).test(
    ...
    (record.windowsX64Asset === null ||
      new RegExp(`^Vincu-Setup-${record.version.replaceAll(".", "\\.")}(?:-x64)?\\.exe$`).test(
        record.windowsX64Asset,
      )) &&
    (record.windowsArm64Asset === null ||
      new RegExp(`^Vincu-Setup-${record.version.replaceAll(".", "\\.")}-arm64\\.exe$`).test(
