...
export function resolveSupervisorLogFile(
  ...
  const envRotateSize = env.VINCU_LOG_ROTATE_SIZE?.trim();
  const envRotateMaxFiles = parseOptionalPositiveInteger(env.VINCU_LOG_ROTATE_COUNT);
  let logPath = path.join(vincuHome, DEFAULT_DAEMON_LOG_FILENAME);
  if (configuredPath) {
    logPath = path.isAbsolute(configuredPath)
      ? configuredPath
      : path.resolve(vincuHome, configuredPath);
  }
