...
function removeZshShellIntegrationRuntimeDir(): void {
  rmSync(join(tmpdir(), `${userInfo().username || "unknown"}-vincu-zsh-${process.pid}`), {
    recursive: true,
    force: true,
  });
}
...
describe.skipIf(isPlatform("win32"))("terminal POSIX-only", () => {
  it("sets zsh wrapper env when spawning zsh", () => {
    const resolvedEnv = buildTerminalEnvironment({
      ...
      env: {
        HOME: "/tmp/vincu-home",
        ZDOTDIR: "/tmp/vincu-zdotdir",
      },
    });

    expect(resolvedEnv.TERM).toBe("xterm-256color");
    expect(resolvedEnv.TERM_PROGRAM).toBe("kitty");
    expect(resolvedEnv.VINCU_ZSH_ZDOTDIR).toBe("/tmp/vincu-zdotdir");
    expect(resolvedEnv.ZDOTDIR).toBe(
      join(tmpdir(), `${userInfo().username || "unknown"}-vincu-zsh-${process.pid}`),
    );
    expect(existsSync(join(resolvedEnv.ZDOTDIR, ".zshenv"))).toBe(true);
    expect(existsSync(join(resolvedEnv.ZDOTDIR, "vincu-integration.zsh"))).toBe(true);
  });

  it("reuses zsh shell integration copied from read-only source files", () => {
    const integrationSourceDir = mkdtempSync(join(tmpdir(), "vincu-zsh-readonly-source-"));
    const tmpHome = mkdtempSync(join(tmpdir(), "vincu-zsh-readonly-home-"));
    ...
    chmodSync(join(integrationSourceDir, "vincu-integration.zsh"), 0o444);
  ...
  describe("terminal title", () => {
    it.skipIf(!hasZsh)("restores the user's ZDOTDIR through the zsh wrapper", async () => {
      ...
      writeFileSync(join(realZdotdir, ".zshenv"), "export VINCU_TEST_REAL_ZDOTDIR=1\n");
      ...
      const session = trackSession(
        await createTerminal({
          ...
          cwd: homeDir,
          command: "/bin/zsh",
          args: ["-c", 'printf \'%s\\n%s\\n\' "${ZDOTDIR-}" "${VINCU_TEST_REAL_ZDOTDIR-}"'],
          env: {
            HOME: homeDir,
            ZDOTDIR: realZdotdir,
          },
    ...
    it.skipIf(!hasZsh)("loads the user's zsh prompt when the integration dir is packaged", () => {
      ...
      temporaryDirs.push(homeDir);
      writeFileSync(join(homeDir, ".zshrc"), "PS1='VINCU_CUSTOM_PROMPT> '\n");

      const fakeAppRoot = join(homeDir, "Vincu.app", "Contents", "Resources");
