...
import {
  resolveVincuCliBinDir,
  resolveVincuCliExecutablePath,
...
it("delivers rapid small writes complete and in order through worker coalescing", async () => {
  ...
  const session = trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      cwd,
      env: { VINCU_TERMINAL_BURST_GATE: burstGatePath },
      ...nodeTerminalCommand(`
      const fs = require("node:fs");
      const gatePath = process.env.VINCU_TERMINAL_BURST_GATE;
...
it("keeps registered cwd env inheritance behind the worker manager interface", async () => {
  ...
  manager.registerCwdEnv({
    cwd,
    env: { VINCU_WORKER_TERMINAL_TEST: "worker-env" },
  });
  trackTerminal(
    await manager.createTerminal({
      workspaceId: "ws-test",
      ...
      require("node:fs").writeFileSync(
        ...
        process.env.VINCU_WORKER_TERMINAL_TEST ?? "",
      );
      setInterval(() => {}, 1000);
    `),
    }),
...
it("injects parent-minted terminal activity env through the worker", async () => {
    ...
    await manager.createTerminal({
      ...
      ...nodeTerminalCommand(`
        require("node:fs").writeFileSync(
          ...
          JSON.stringify({
            terminalId: process.env.VINCU_TERMINAL_ID,
            token: process.env.VINCU_ACTIVITY_TOKEN,
            url: process.env.VINCU_TERMINAL_ACTIVITY_URL,
            hookCli: process.env.VINCU_HOOK_CLI,
  ...
  const vincuCliBinDir = resolveVincuCliBinDir();
  const vincuCliPath = resolveVincuCliExecutablePath();
  expect(vincuCliBinDir).not.toBeNull();
  expect(vincuCliPath).not.toBeNull();
  ...
  expect(env.hookCli).toBe(vincuCliPath);
  ...
  expect(env.path?.split(delimiter)[0]).toBe(vincuCliBinDir);
});
