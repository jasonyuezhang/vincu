    ...
    return provider.events.every((event) =>
      normalizeMatchers(hooks[event.event]).some((entry) =>
        normalizeCommandHooks(entry.hooks).some((hook) =>
          hasVincuCommands(hook, install.hookMarker),
        ),
      ),
    );
  },
...
function removeVincuHooks(value: unknown, marker: string): CodexMatcherGroup[] {
...
function hasVincuCommands(hook: CodexCommandHook, marker: string): boolean {
  return (
