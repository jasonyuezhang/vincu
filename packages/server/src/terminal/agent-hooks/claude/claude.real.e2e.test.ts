...
import { createTerminal, resolveVincuCliBinDir, type TerminalSession } from "../../terminal.js";
...
describe.skipIf(!claudeAvailability.available)(
  ...
  () => {
    it("posts running then idle through the installed Claude config hooks", async () => {
      ...
      const configDir = createTempDir("vincu-real-claude-config-");
      const cwd = createTempDir("vincu-real-claude-cwd-");
      const vincuCliBinDir = resolveVincuCliBinDir();
      if (!vincuCliBinDir) {
        throw new Error("Could not resolve vincu CLI bin directory");
      }
      ...
      try {
        const claude = pty.spawn(
          ...
          ["--settings", join(configDir, "settings.json"), "-p", "reply with: done"],
          {
            ...
            env: {
              ...process.env,
              VINCU_TERMINAL_ID: terminalId,
              VINCU_ACTIVITY_TOKEN: token,
              VINCU_TERMINAL_ACTIVITY_URL: recorder.url,
              PATH: [vincuCliBinDir, process.env.PATH].filter(isString).join(delimiter),
            },
          },
        );
    ...
    it("clears activity for an interrupted interactive Claude turn through TerminalSession", async () => {
      ...
      const configDir = createTempDir("vincu-real-claude-interrupt-config-");
      const vincuCliBinDir = resolveVincuCliBinDir();
      if (!vincuCliBinDir) {
        throw new Error("Could not resolve vincu CLI bin directory");
      }
