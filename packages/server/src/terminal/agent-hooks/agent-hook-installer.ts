...
export function buildAgentHookShellCommand<TConfig>(
  ...
  const hookCommand = `"\${VINCU_HOOK_CLI:-vincu}" hooks ${shellToken(provider.id)} ${shellToken(event.event)}`;
  return `if [ -n "$VINCU_TERMINAL_ID" ]; then ${hookCommand}; fi`;
}
...
export function buildAgentHookWindowsCommand<TConfig>(
  ...
  return `if defined VINCU_TERMINAL_ID (if defined VINCU_HOOK_CLI ("%VINCU_HOOK_CLI%" ${hookArgs}) else (vincu ${hookArgs})) else (exit /b 0)`;
}
