...
import { parseGitRemoteLocation, type GitRemoteLocation } from "@getvincu/protocol/git-remote";
...
export function createForgeResolver(options: CreateForgeResolverOptions = {}): ForgeResolver {
  ...
  // from outside Vincu — `git remote add`/`set-url` run in a terminal — is
  // still picked up within a cycle instead of staying pinned forever; a Vincu
