...
} from "./vincu-config-file.js";

describe("vincu config file substrate", () => {
  let tempDir: string;

  beforeEach(() => {
    tempDir = realpathSync(mkdtempSync(join(tmpdir(), "vincu-config-file-test-")));
  });
  ...
  it("returns null config and revision when vincu.json is missing", () => {
    const result = readVincuConfigForEdit(tempDir);

    expect(result).toEqual({ ok: true, config: null, revision: null });
  });

  it("returns invalid_project_config for invalid JSON", () => {
    writeFileSync(join(tempDir, "vincu.json"), "{ invalid json\n");
  ...
  it("preserves raw lifecycle string and array forms with a revision token", () => {
    ...
    expect(result).toEqual({
      ...
      revision: statVincuConfigPath(tempDir),
    });
  });
  ...
  it("writes pretty JSON with a trailing newline when revision matches", () => {
    writeFileSync(join(tempDir, "vincu.json"), JSON.stringify({ worktree: { setup: "old" } }));
    const expectedRevision = statVincuConfigPath(tempDir);
    ...
    expect(readFileSync(join(tempDir, "vincu.json"), "utf8")).toBe(
      '{\n  "worktree": {\n    "setup": "npm install"\n  }\n}\n',
    );
  });
  ...
  it.skipIf(isPlatform("win32"))(
    ...
    () => {
      writeFileSync(join(tempDir, "vincu.json"), JSON.stringify({ worktree: { setup: "old" } }));
      const expectedRevision = statVincuConfigPath(tempDir);
      writeFileSync(join(tempDir, "vincu.json"), JSON.stringify({ worktree: { setup: "new" } }));
      const currentRevision = statVincuConfigPath(tempDir);
  ...
  it("round-trips unknown top-level, worktree, and script-entry fields", () => {
    ...
    expect(readVincuConfigForEdit(tempDir)).toEqual({
      ok: true,
      config,
      revision: statVincuConfigPath(tempDir),
    });
  ...
  it("creates vincu.json when the file is still missing and expected revision is null", () => {
    ...
    expect(result).toEqual({
      ok: true,
      config: { scripts: { dev: { command: "npm run dev" } } },
      revision: statVincuConfigPath(join(tempDir, "nested")),
    });
  });
});
