...
import { base64EncryptedWireByteLength } from "@getvincu/relay";
import {
...
import {
  getVincuWorktreeMetadataPath,
  readVincuWorktreeMetadata,
  writeVincuWorktreeMetadata,
...
function createGitHubServiceForStatus(
  ...
  return {
    ...
    getPullRequest: async () => ({
      number: 1,
      title: "PR",
      url: "https://github.com/getvincu/vincu/pull/1",
      state: "OPEN",
      body: null,
      baseRefName: "main",
      headRefName: "feature",
...
async function readPullRequestLookupTargetFromFacts(
  ...
  const facts = await getCheckoutSnapshotFacts(repoDir, { vincuHome });
  if (!facts.isGit) {
...
describe("checkout git utilities", () => {
  ...
  it("reuses checkout snapshot facts across status, shortstat, and PR status reads", async () => {
    ...
    const facts = await getCheckoutSnapshotFacts(repoDir, { vincuHome });
    const status = await getCheckoutStatus(repoDir, { vincuHome, facts });
    const shortstat = await getCheckoutShortstat(repoDir, { vincuHome, facts }, { force: true });
    ...
    __resetPullRequestStatusCacheForTests();
    startGitCommandMetrics();
    const statusWithFacts = await getCheckoutStatus(repoDir, { vincuHome, facts });
...
`;
  ...
  it("returns checkout root metadata for normal repos", async () => {
    ...
    expect(status.isVincuOwnedWorktree).toBe(false);
  ...
  it("reports a PR worktree as not ahead when its branch is pushed to the configured PR remote", async () => {
    ...
    commitFile(repoDir, "feature.txt", "feature\n", "feature commit");
    execFileSync("git", ["remote", "add", "vincu-pr-1285", prRemoteDir], { cwd: repoDir });
    execFileSync(
      "git",
      ["push", "vincu-pr-1285", "HEAD:refs/heads/open-button-targets-active-file"],
      { cwd: repoDir },
    );
    execFileSync(
      "git",
      ["config", "branch.aaronzhongg/open-button-targets-active-file.remote", "vincu-pr-1285"],
      {
        cwd: repoDir,
      },
    );
  ...
  it("reports a PR worktree as behind when its configured PR remote has newer commits", async () => {
    ...
    execFileSync("git", ["fetch", "vincu-pr-1285"], { cwd: repoDir });
  ...
  it("does not report full history as unpushed for fresh no-track Vincu worktrees", async () => {
    setupRemoteTrackingMain(repoDir, tempDir);
    ...
    const status = await getCheckoutStatus(worktree.worktreePath, { vincuHome });
    expect(status).toMatchObject({
      isGit: true,
  ...
  it("resolves the Git common directory once when reading Vincu worktree facts", async () => {
    ...
    startGitCommandMetrics();
    const facts = await getCheckoutSnapshotFacts(result.worktreePath, { vincuHome });
    const metrics = stopGitCommandMetrics();
  ...
  it("handles status/diff/commit in a .vincu worktree", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
    ...
    const status = await getCheckoutStatus(result.worktreePath, { vincuHome });
    expect(status.isGit).toBe(true);
    expect(realpathSync.native(status.repoRoot)).toBe(realpathSync.native(result.worktreePath));
    expect(status.isDirty).toBe(true);
    expect(status.isVincuOwnedWorktree).toBe(true);
    expect(realpathSync.native(status.mainRepoRoot ?? "")).toBe(realpathSync.native(repoDir));

    const diff = await getCheckoutDiff(result.worktreePath, { mode: "uncommitted" }, { vincuHome });
    expect(diff.diff).toContain("-hello");
    expect(diff.diff).toContain("+worktree change");

    await commitAll(result.worktreePath, "worktree update");

    const cleanStatus = await getCheckoutStatus(result.worktreePath, { vincuHome });
    expect(cleanStatus.isDirty).toBe(false);
  ...
  it("returns checkout root metadata for .vincu worktrees", async () => {
    const result = await createLegacyWorktreeForTest({
      branchName: "main",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("detects plain git worktrees from git alone", async () => {
    ...
    const status = await getCheckoutStatus(worktreeDir, { vincuHome });
    expect(status.isGit).toBe(true);
  ...
  it("merges the current branch into base from a worktree checkout", async () => {
    ...
    await mergeToBase(worktree.worktreePath, { baseRef: "main" }, { vincuHome });
    ...
    const statusAfterMerge = await getCheckoutStatus(worktree.worktreePath, { vincuHome });
  ...
  it("reports the base worktree cwd when merge-to-base mutates a separate checkout", async () => {
    ...
    const mutatedCwd = await mergeToBase(featureWorktree.worktreePath, {}, { vincuHome });
  ...
  it("pushes the current branch to its configured upstream", async () => {
    ...
    execFileSync("git", ["remote", "add", "vincu-pr-526", prRemoteDir], { cwd: repoDir });
    execFileSync("git", ["checkout", "-b", "therainisme/main"], { cwd: repoDir });
    execFileSync("git", ["fetch", "vincu-pr-526", "main"], { cwd: repoDir });
    execFileSync("git", ["config", "branch.therainisme/main.remote", "vincu-pr-526"], {
      cwd: repoDir,
    });
    ...
    expect(upstream).toBe("vincu-pr-526/main");
  });
  ...
  it("pushes the current branch to its configured push remote", async () => {
    ...
    execFileSync("git", ["remote", "add", "vincu-pr-526", prRemoteDir], { cwd: repoDir });
    execFileSync("git", ["checkout", "-b", "therainisme/main"], { cwd: repoDir });
    execFileSync("git", ["config", "branch.therainisme/main.pushRemote", "vincu-pr-526"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.vincu-pr-526.push", "HEAD:refs/heads/main"], {
      cwd: repoDir,
    });
    writeFileSync(join(repoDir, "fork-pr.txt"), "fork pr edit\n");
    execFileSync("git", ["add", "fork-pr.txt"], { cwd: repoDir });
    ...
    const trackedPrRemoteHead = execFileSync(
      "git",
      ["rev-parse", "refs/remotes/vincu-pr-526/main"],
      {
        cwd: repoDir,
      },
    )
    ...
    expect(prRemoteMain).toBe(localHead);
    expect(getBranchUpstream(repoDir)).toBe("vincu-pr-526/main");
  ...
  it("refreshes the tracked ref after pushing through a configured push remote", async () => {
    ...
    execFileSync("git", ["checkout", "-b", "feature"], { cwd: repoDir });
    execFileSync("git", ["push", "-u", "origin", "feature"], { cwd: repoDir });
    execFileSync("git", ["remote", "add", "vincu-pr-1790", originDir], { cwd: repoDir });
    execFileSync("git", ["config", "branch.feature.pushRemote", "vincu-pr-1790"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.vincu-pr-1790.push", "HEAD:refs/heads/feature"], {
      cwd: repoDir,
    });
  ...
  it("disables GitHub features when gh is unavailable", async () => {
    execFileSync("git", ["remote", "add", "origin", "https://github.com/getvincu/vincu.git"], {
      cwd: repoDir,
    });
          ...
          checks: [
            {
              name: "server-tests",
              status: "success",
              url: "https://github.com/getvincu/vincu/actions/runs/123",
              workflow: "Server CI",
              duration: "2m 14s",
            },
          ],
  ...
  it("uses an origin tracked head when the local branch name differs", async () => {
    ...
    const lookupTarget = await readPullRequestLookupTargetFromFacts(repoDir, vincuHome);
  ...
  it("reconciles a PR worktree lookup from current branch tracking", async () => {
    ...
    const workspaceDir = join(vincuHome, "worktrees", "repo", "pr-worktree");
    mkdirSync(join(vincuHome, "worktrees", "repo"), { recursive: true });
    ...
    writeVincuWorktreeMetadata(workspaceDir, {
      baseRefName: "main",
    ...
    expect(await readPullRequestLookupTargetFromFacts(workspaceDir, vincuHome)).toMatchObject({
      headRef: "old-change",
      headRepositoryOwner: "contributor",
    });
    ...
    startGitCommandMetrics();
    const switchedTarget = await readPullRequestLookupTargetFromFacts(workspaceDir, vincuHome);
  ...
  it("does not apply ambiguous legacy PR metadata to a suffixed branch", async () => {
    ...
    const workspaceDir = join(vincuHome, "worktrees", "repo", "legacy-pr-worktree");
    mkdirSync(join(vincuHome, "worktrees", "repo"), { recursive: true });
    ...
    const lookupTarget = await readPullRequestLookupTargetFromFacts(workspaceDir, vincuHome);
  ...
  it("does not apply a legacy fork hint to an ownerless branch with the same head", async () => {
    ...
    const workspaceDir = join(vincuHome, "worktrees", "repo", "legacy-fork-worktree");
    mkdirSync(join(vincuHome, "worktrees", "repo"), { recursive: true });
    execFileSync("git", ["worktree", "add", workspaceDir, "contributor/old-change"], {
      cwd: repoDir,
    });
    ...
    const forkFacts = await getCheckoutSnapshotFacts(workspaceDir, { vincuHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "legacy-fork-branch" },
      { vincuHome, facts: forkFacts },
    );
    ...
    const ownerlessFacts = await getCheckoutSnapshotFacts(workspaceDir, { vincuHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "ownerless-same-head" },
      { vincuHome, facts: ownerlessFacts },
    );
  ...
  it("recognizes a normalized GitHub owner branch from legacy Enterprise metadata", async () => {
    ...
    const workspaceDir = join(vincuHome, "worktrees", "repo", "legacy-enterprise-worktree");
    mkdirSync(join(vincuHome, "worktrees", "repo"), { recursive: true });
    ...
    const facts = await getCheckoutSnapshotFacts(workspaceDir, { vincuHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
    ...
    const repointedFacts = await getCheckoutSnapshotFacts(workspaceDir, { vincuHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "repointed-enterprise-owner" },
      { vincuHome, facts: repointedFacts },
    );
  ...
  it("keeps a ref-only change request bound across rename but not branch switch", async () => {
    ...
    const workspaceDir = join(vincuHome, "worktrees", "repo", "gitlab-mr-worktree");
    mkdirSync(join(vincuHome, "worktrees", "repo"), { recursive: true });
    ...
    expect(readVincuWorktreeMetadata(workspaceDir)?.changeRequestLookupTarget).toMatchObject({
      localBranchName: "feature/renamed",
    });
    const renamedFacts = await getCheckoutSnapshotFacts(workspaceDir, { vincuHome });
    const renamedStatus = await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "renamed-change-request" },
      { vincuHome, facts: renamedFacts },
    );
    ...
    const switchedFacts = await getCheckoutSnapshotFacts(workspaceDir, { vincuHome });
    await getPullRequestStatus(
      workspaceDir,
      forge,
      { force: true, reason: "switched-after-rename" },
      { vincuHome, facts: switchedFacts },
    );
  ...
  it("keeps fork identity when the local and tracked branch names match", async () => {
    ...
    execFileSync("git", ["remote", "add", "contributor", "git@github.com:contributor/vincu.git"], {
      cwd: repoDir,
    });
  ...
  it("does not attach an owner when the tracked remote is the same GitHub repository", async () => {
    ...
    execFileSync("git", ["remote", "add", "origin", "git@github.com:getvincu/vincu.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["remote", "add", "upstream", "https://github.com/getvincu/vincu.git"], {
      cwd: repoDir,
    });
  ...
  it("keeps the fork owner when same-repo comparison is indeterminate", async () => {
    ...
    execFileSync("git", ["remote", "add", "vincu-pr-345", "git@github.com:chethanuk/vincu.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "branch.chethanuk/main.remote", "vincu-pr-345"], {
      cwd: repoDir,
    });
  ...
  it("uses the configured push remote for fork PR lookup when upstream is absent", async () => {
    ...
    execFileSync("git", ["config", "branch.chethanuk/main.pushRemote", "vincu-pr-345"], {
      cwd: repoDir,
    });
    execFileSync("git", ["config", "remote.vincu-pr-345.push", "HEAD:refs/heads/main"], {
      cwd: repoDir,
    });
    const requestedTargets: RequestedPullRequestTarget[] = [];
    const github = createGitHubServiceRecordingPullRequestTargets({ requestedTargets });

    const factsTarget = await readPullRequestLookupTargetFromFacts(repoDir, vincuHome);
    await getPullRequestStatus(
      repoDir,
      github,
  ...
  it("keeps the local branch lookup when a fork tracks the upstream base branch", async () => {
    execFileSync("git", ["checkout", "-b", "local-feature"], { cwd: repoDir });
    execFileSync("git", ["remote", "add", "origin", "git@github.com:contributor/vincu.git"], {
      cwd: repoDir,
    });
    execFileSync("git", ["remote", "add", "upstream", "https://github.com/getvincu/vincu.git"], {
      cwd: repoDir,
  ...
  it("uses the tracked fork branch for PR worktree status lookup", async () => {
    ...
    const github = createGitHubServiceRecordingPullRequestTargets({
      ...
      statusOverrides: {
        number: 345,
        url: "https://github.com/getvincu/vincu/pull/345",
      },
    });
  ...
  it("returns closed-unmerged PR status without marking it as merged", async () => {
    ...
    const status = await getPullRequestStatus(
      ...
      createGitHubServiceForStatus(
        createPullRequestStatus({
          url: "https://github.com/getvincu/vincu/pull/999",
  ...
  it("does not reuse a PR status cache entry after HEAD changes on the same branch", async () => {
    ...
    github.getCurrentPullRequestStatus = async (options) => {
      ...
      return createPullRequestStatus({
        url: `https://github.com/getvincu/vincu/pull/${requestedShas.length}`,
      });
    };
  ...
  it("expires cached PR status after the TTL", async () => {
    ...
    try {
      ...
      github.getCurrentPullRequestStatus = async () => {
        callCount += 1;
        return createPullRequestStatus({
          url: `https://github.com/getvincu/vincu/pull/${callCount}`,
        });
      };
      const first = await getPullRequestStatus(repoDir, github);
      await sleep(80);
  ...
  it("uses stored baseRefName for Vincu worktrees (no heuristics)", async () => {
    ...
    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { vincuHome });
  ...
  it("names both refs when a requested base ref does not match the stored one", async () => {
    ...
    await expect(
      getCheckoutDiff(worktree.worktreePath, { mode: "base", baseRef: "other" }, { vincuHome }),
    ).rejects.toThrow("Base ref mismatch: stored refs/heads/main, requested other");
  });

  it("excludes dirty working tree changes from Vincu worktree base diffs", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  it("merges to stored baseRefName when baseRef is not provided", async () => {
    ...
    // Create a Vincu worktree configured to use develop as base.
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "develop",
    ...
    await mergeToBase(worktree.worktreePath, {}, { vincuHome });
  ...
  it("falls back to the repository default branch for base-dependent operations when metadata is missing", async () => {
    ...
    const metadataPath = getVincuWorktreeMetadataPath(worktree.worktreePath);
    rmSync(metadataPath, { force: true });

    const baseDiff = await getCheckoutDiff(worktree.worktreePath, { mode: "base" }, { vincuHome });
    expect(baseDiff.diff).toContain("feature.txt");

    const shortstat = await getCheckoutShortstat(worktree.worktreePath, { vincuHome });
    expect(shortstat).toEqual({ additions: 1, deletions: 0 });
  });

  it("falls back to plain git checkout status when Vincu worktree metadata is missing", async () => {
    const worktree = await createLegacyWorktreeForTest({
      branchName: "feature",
      cwd: repoDir,
      baseBranch: "main",
  ...
  describe("parseWorktreeList", () => {
    it("parses porcelain worktree output", () => {
      const output = [
        ...
        "",
        "worktree /home/user/.vincu/worktrees/feature",
      ...
      expect(entries[1]).toEqual({
        path: "/home/user/.vincu/worktrees/feature",
  ...
  describe("isVincuWorktreePath", () => {
    it("matches Unix .vincu/worktrees/ paths", () => {
      expect(isVincuWorktreePath("/home/user/.vincu/worktrees/feature")).toBe(true);
    });

    it("matches Windows .vincu\\worktrees\\ paths", () => {
      expect(isVincuWorktreePath("C:\\Users\\dev\\.vincu\\worktrees\\feature")).toBe(true);
    });

    it("matches worktrees under a custom VINCU_HOME", () => {
      const customVincuHome = process.platform === "win32" ? "C:\\vincu" : "/var/lib/vincu";
      const worktreePath =
        process.platform === "win32"
          ? win32.join(customVincuHome, "worktrees", "project", "feature")
          : `${customVincuHome}/worktrees/project/feature`;

      expect(
        isVincuWorktreePath(worktreePath, {
          vincuHome: customVincuHome,
        }),
      ).toBe(true);
    });

    it("rejects paths without .vincu/worktrees segment", () => {
      expect(isVincuWorktreePath("/home/user/repo")).toBe(false);
      expect(isVincuWorktreePath("C:\\Users\\dev\\repo")).toBe(false);
    });
  });
