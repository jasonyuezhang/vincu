...
import type { CheckoutCommit, CheckoutCommitFile } from "@getvincu/protocol/messages";
import { parseGitHubRemoteIdentity, parseGitRemoteLocation } from "@getvincu/protocol/git-remote";
import { maxBase64EncryptedPlaintextByteLength } from "@getvincu/relay";
...
import { isVincuOwnedWorktreeCwd, resolveVincuWorktreesBaseRoot } from "./worktree.js";
import {
  branchNameFromRef,
  getVincuWorktreeChangeRequestHintForBranch,
  type VincuWorktreeMetadata,
  readVincuWorktreeMetadata,
  rebindVincuWorktreeChangeRequestHint,
...
export interface CheckoutStatusGitNonVincu {
  isGit: true;
  repoRoot: string;
...
export interface CheckoutStatusGitVincu {
  isGit: true;
  repoRoot: string;
  mainRepoRoot: string;
  currentBranch: string | null;
...
export type CheckoutStatusGit = CheckoutStatusGitNonVincu | CheckoutStatusGitVincu;
...
export type CheckoutSnapshotFacts =
  ...
  | {
      ...
      vincuWorktree: VincuWorktreeForCwd;
...
async function getMainRepoRootFromCommonDir(
  ...
  const nonBareNonVincu = worktrees.filter(
    (wt) =>
      !wt.isBare &&
      !isVincuWorktreePath(wt.path, {
        vincuHome: context?.vincuHome,
        worktreesRoot: context?.worktreesRoot,
      }),
  );
  const childrenOfBareRepo = nonBareNonVincu.filter((wt) => isDescendantPath(wt.path, normalized));
  const mainChild = childrenOfBareRepo.find((wt) => basename(wt.path) === "main");
  return mainChild?.path ?? childrenOfBareRepo[0]?.path ?? nonBareNonVincu[0]?.path ?? normalized;
}
...
/** Check whether a path is under Vincu's worktree root. */
export function isVincuWorktreePath(
  p: string,
  options?: { vincuHome?: string; worktreesRoot?: string },
): boolean {
  if (options?.worktreesRoot || options?.vincuHome) {
    return isDescendantPath(p, resolveVincuWorktreesBaseRoot(options));
  }
  return /[/\\]\.vincu[/\\]worktrees[/\\]/.test(p);
}
...
export async function renameCurrentBranch(
  ...
  if (currentBranch) {
    rebindVincuWorktreeChangeRequestHint(worktreeRoot, previousBranch, currentBranch);
  }
  return { previousBranch, currentBranch };
}

type VincuWorktreeForCwd =
  | { isVincuOwnedWorktree: false }
  | { isVincuOwnedWorktree: true; worktreeRoot: string };

interface VincuWorktreeLookupOptions {
  context?: CheckoutContext;
  knownWorktreeRoot?: string | null;
  knownGitCommonDir?: string | null;
}

async function getVincuWorktreeForCwd(
  cwd: string,
  options: VincuWorktreeLookupOptions = {},
): Promise<VincuWorktreeForCwd> {
  // Fast-path reject: non-worktree paths do not need expensive ownership checks.
  if (!/[\\/]worktrees[\\/]/.test(cwd)) {
    return { isVincuOwnedWorktree: false };
  }

  const ownership = await isVincuOwnedWorktreeCwd(cwd, {
    vincuHome: options.context?.vincuHome,
...
function storedBaseRefFromMetadata(metadata: VincuWorktreeMetadata | null): string | null {
  return metadata?.baseRef ?? metadata?.baseRefName ?? null;
}

function readVincuWorktreeBaseRef(worktreeRoot: string): string | null {
  return storedBaseRefFromMetadata(readVincuWorktreeMetadata(worktreeRoot));
}
...
async function getStoredBaseRefForCwd(
  ...
  const vincuWorktree = await getVincuWorktreeForCwd(cwd, { context });
  if (!vincuWorktree.isVincuOwnedWorktree) {
    return null;
  }

  return readVincuWorktreeBaseRef(vincuWorktree.worktreeRoot);
}
...
async function inspectCheckoutContext(
  ...
  const vincuWorktree = await getVincuWorktreeForCwd(cwd, {
    context,
...
function buildPullRequestLookupTargetFromMetadata(
  ...
  const target = getVincuWorktreeChangeRequestHintForBranch(metadata, currentBranch);
  if (!target) {
    return null;
  }
  return {
...
export async function getCheckoutSnapshotFacts(
  ...
  const vincuWorktreeMetadata = inspected.vincuWorktree.isVincuOwnedWorktree
    ? readVincuWorktreeMetadata(inspected.vincuWorktree.worktreeRoot)
    : null;
  const storedBaseRef = storedBaseRefFromMetadata(vincuWorktreeMetadata);
  ...
  return {
    ...
    vincuWorktree: inspected.vincuWorktree,
    storedBaseRef,
    resolvedBaseRef,
    mainRepoRoot,
    comparisonBaseRef,
...
// Temporary until diffs load lazily per file. The Vincu relay's 32 MiB frame limit is
...
export async function getCheckoutStatus(
  ...
  const vincuWorktree = facts.vincuWorktree;
  ...
  if (vincuWorktree.isVincuOwnedWorktree && baseRef) {
    return {
      isGit: true,
