...
import type { VincuConfig } from "@getvincu/protocol/vincu-config-schema";
import { getVincuWorktreeMetadataPath } from "./worktree-metadata.js";
import {
  getCheckoutDiff,
  getCheckoutStatus,
  listCheckoutCommits,
...
function loadConfigForTest(repoRoot: string): VincuConfig | null {
  const result = readVincuConfig(repoRoot);
...
describe.skipIf(isPlatform("win32"))("worktree POSIX-only", () => {
  describe("createWorktree", () => {
    ...
    it("creates a worktree for the current branch (main)", async () => {
      ...
      expect(result.worktreePath).toBe(join(vincuHome, "worktrees", projectHash, "hello-world"));
      expect(existsSync(result.worktreePath)).toBe(true);
      expect(existsSync(join(result.worktreePath, "file.txt"))).toBe(true);
      const metadataPath = getVincuWorktreeMetadataPath(result.worktreePath);
    ...
    it("creates and owns worktrees under a configured root", async () => {
      ...
      await expect(
        isVincuOwnedWorktreeCwd(result.worktreePath, { vincuHome, worktreesRoot }),
      ).resolves.toMatchObject({ allowed: true, worktreeRoot: join(worktreesRoot, projectHash) });
      await expect(
        isVincuOwnedWorktreeCwd(result.worktreePath, { vincuHome }),
      ).resolves.toMatchObject({ allowed: false });

      const worktrees = await listVincuWorktrees({ cwd: repoDir, vincuHome, worktreesRoot });
    ...
    it.skip("detects vincu-owned worktrees across realpath differences (macOS /var vs /private/var)", async () => {
      ...
      const varVincuHome = join(varTempDir, "vincu-home");
      ...
      const ownership = await isVincuOwnedWorktreeCwd(privateWorktreePath, {
        vincuHome: varVincuHome,
      });
    ...
    it("reports repoRoot as the repository root for vincu-owned worktrees", async () => {
      const result = await createLegacyWorktreeForTest({
        branchName: "main",
        cwd: repoDir,
        baseBranch: "main",
      ...
      const ownership = await isVincuOwnedWorktreeCwd(result.worktreePath, { vincuHome });
      expect(ownership.allowed).toBe(true);
    ...
    it("treats non-git directories as non-worktrees without throwing", async () => {
      ...
      const ownership = await isVincuOwnedWorktreeCwd(nonGitDir, { vincuHome });
    ...
    it("creates a worktree with a new branch", async () => {
      ...
      expect(result.worktreePath).toBe(join(vincuHome, "worktrees", projectHash, "my-feature"));
      expect(existsSync(result.worktreePath)).toBe(true);
    ...
    it("fetches a GitHub PR branch, checks it out, writes metadata, and runs setup", async () => {
      ...
      writeFileSync(
        join(remoteCloneDir, "vincu.json"),
    ...
    it("uses the selected local or origin ref when both exist", async () => {
      ...
      const localStatus = await getCheckoutStatus(localResult.worktreePath, { vincuHome });
      ...
      await expect(
        getCheckoutDiff(localResult.worktreePath, { mode: "base", baseRef: "main" }, { vincuHome }),
      ).resolves.toMatchObject({ diff: "" });
      expect(readFileSync(join(originResult.worktreePath, "file.txt"), "utf8")).toBe(
        "from-origin\n",
      );
      expect(
        JSON.parse(readFileSync(getVincuWorktreeMetadataPath(originResult.worktreePath), "utf8")),
    ...
    it("records the branch name when the base is on a remote other than origin", async () => {
      ...
      expect(
        JSON.parse(readFileSync(getVincuWorktreeMetadataPath(result.worktreePath), "utf8")),
      ...
      await expect(
        mergeToBase(result.worktreePath, { baseRef: "main" }, { vincuHome }),
      ).rejects.toThrow(
      ...
      await mergeFromBase(result.worktreePath, { baseRef: "main" }, { vincuHome });
      ...
      await expect(
        getCheckoutDiff(result.worktreePath, { mode: "base", baseRef: "main" }, { vincuHome }),
      ).rejects.toThrow("Base ref not found: refs/remotes/upstream/main");
      await expect(
        mergeFromBase(result.worktreePath, { baseRef: "main" }, { vincuHome }),
      ).rejects.toThrow("Base ref not found: refs/remotes/upstream/main");
      await expect(
        listCheckoutCommits({ cwd: result.worktreePath, context: { vincuHome } }),
      ).rejects.toThrow("Base ref not found: refs/remotes/upstream/main");
    });
    ...
    it("handles branch name collision by adding suffix", async () => {
      ...
      expect(result.worktreePath).toBe(join(vincuHome, "worktrees", projectHash, "hello"));
      expect(existsSync(result.worktreePath)).toBe(true);
    ...
    it("runs setup commands from vincu.json", async () => {
      // Create vincu.json with setup commands
      const vincuConfig = {
        worktree: {
          setup: [
            'echo "source=$VINCU_SOURCE_CHECKOUT_PATH" > setup.log',
            'echo "root_alias=$VINCU_ROOT_PATH" >> setup.log',
            'echo "worktree=$VINCU_WORKTREE_PATH" >> setup.log',
            'echo "branch=$VINCU_BRANCH_NAME" >> setup.log',
            'echo "port=$VINCU_WORKTREE_PORT" >> setup.log',
          ],
        },
      };
      writeFileSync(join(repoDir, "vincu.json"), JSON.stringify(vincuConfig));
      execFileSync("git", ["add", "vincu.json"], { cwd: repoDir });
      execFileSync("git", ["-c", "commit.gpgsign=false", "commit", "-m", "add vincu.json"], {
        cwd: repoDir,
      });
    ...
    it("runs string setup scripts from vincu.json as a single shell command", async () => {
      const vincuConfig = {
        worktree: {
    ...
    it("runs setup commands with the daemon PATH instead of login profile PATH", async () => {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          worktree: {
            setup: "command -v vincu-shim >/dev/null && vincu-shim ok > setup-path.log",
          },
        }),
      );
      ...
      try {
        await runWorktreeSetupCommands({
          ...
          runtimeEnv: {
            VINCU_SOURCE_CHECKOUT_PATH: repoDir,
            VINCU_ROOT_PATH: repoDir,
            VINCU_WORKTREE_PATH: repoDir,
            VINCU_BRANCH_NAME: "main",
            VINCU_WORKTREE_PORT: "12345",
          },
        });
      } finally {
      ...
      writeFileSync(
        ...
        JSON.stringify({
          worktree: {
            ...
            teardown: [
              'echo "first" > "$VINCU_SOURCE_CHECKOUT_PATH/teardown-array.log"',
              null,
              "",
              'echo "second" >> "$VINCU_SOURCE_CHECKOUT_PATH/teardown-array.log"',
            ],
          },
        }),
      );
    ...
    it("reuses persisted worktree runtime port across resolutions", async () => {
      ...
      expect(second.VINCU_WORKTREE_PORT).toBe(first.VINCU_WORKTREE_PORT);
    });
    ...
    it("fails runtime env resolution when persisted port is in use", async () => {
      ...
      const port = Number(env.VINCU_WORKTREE_PORT);
    ...
    it("cleans up worktree if setup command fails", async () => {
      // Create vincu.json with failing setup command
      const vincuConfig = {
        worktree: {
          setup: ["exit 1"],
        },
      ...
      const expectedWorktreePath = join(vincuHome, "worktrees", "test-repo", "fail-test");
    ...
    it("reads worktree terminal specs from vincu.json with optional name", async () => {
      const vincuConfig = {
        worktree: {
          terminals: [
    ...
    it("seeds an uncommitted vincu.json from the main repo into a new worktree", async () => {
      writeFileSync(
        join(repoDir, "vincu.json"),
      ...
      const worktreeConfigPath = join(result.worktreePath, "vincu.json");
    ...
    it("does not overwrite a committed vincu.json with uncommitted edits in the main repo", async () => {
      writeFileSync(
        join(repoDir, "vincu.json"),
    ...
    it("creates a worktree without error when no vincu.json exists in the main repo", async () => {
      const result = await createLegacyWorktreeForTest({
        cwd: repoDir,
      ...
      expect(existsSync(join(result.worktreePath, "vincu.json"))).toBe(false);
    });
  });

  describe("vincu worktree manager", () => {
    let tempDir: string;
    let repoDir: string;
    let vincuHome: string;
    ...
    it("isolates worktree roots for repositories that share the same directory name", async () => {
      ...
      const repoAWorktrees = await listVincuWorktrees({ cwd: repoA, vincuHome });
      const repoBWorktrees = await listVincuWorktrees({ cwd: repoB, vincuHome });
    ...
    it("lists and deletes vincu worktrees under ~/.vincu/worktrees/{hash}", async () => {
      ...
      const worktrees = await listVincuWorktrees({ cwd: repoDir, vincuHome });
      const paths = worktrees.map((worktree) => worktree.path).sort();
      expect(paths).toEqual([first.worktreePath, second.worktreePath].sort());

      await deleteVincuWorktree({ cwd: repoDir, worktreePath: first.worktreePath, vincuHome });
      expect(existsSync(first.worktreePath)).toBe(false);

      const remaining = await listVincuWorktrees({ cwd: repoDir, vincuHome });
      expect(remaining.map((worktree) => worktree.path)).toEqual([second.worktreePath]);
    });

    it("deletes a vincu worktree even when given a subdirectory path", async () => {
      ...
      await deleteVincuWorktree({ cwd: repoDir, worktreePath: nestedDir, vincuHome });
    ...
    it("runs teardown commands from vincu.json before deleting a worktree", async () => {
      const vincuConfig = {
        worktree: {
          teardown: [
            'echo "source=$VINCU_SOURCE_CHECKOUT_PATH" > "$VINCU_SOURCE_CHECKOUT_PATH/teardown.log"',
            'echo "root_alias=$VINCU_ROOT_PATH" >> "$VINCU_SOURCE_CHECKOUT_PATH/teardown.log"',
            'echo "worktree=$VINCU_WORKTREE_PATH" >> "$VINCU_SOURCE_CHECKOUT_PATH/teardown.log"',
            'echo "branch=$VINCU_BRANCH_NAME" >> "$VINCU_SOURCE_CHECKOUT_PATH/teardown.log"',
            'echo "port=$VINCU_WORKTREE_PORT" >> "$VINCU_SOURCE_CHECKOUT_PATH/teardown.log"',
          ],
        },
      };
      writeFileSync(join(repoDir, "vincu.json"), JSON.stringify(vincuConfig));
      ...
      await deleteVincuWorktree({ cwd: repoDir, worktreePath: created.worktreePath, vincuHome });
      expect(existsSync(created.worktreePath)).toBe(false);
      ...
      expect(teardownLog).toContain(`port=${runtimeEnv.VINCU_WORKTREE_PORT}`);
    });

    it("runs string teardown scripts from vincu.json as a single shell command", async () => {
      const vincuConfig = {
        worktree: {
          teardown:
            'cleanup_message="teardown string"\necho "$cleanup_message" > "$VINCU_SOURCE_CHECKOUT_PATH/teardown.log"',
        },
      };
      writeFileSync(join(repoDir, "vincu.json"), JSON.stringify(vincuConfig));
      execFileSync("git", ["add", "vincu.json"], { cwd: repoDir });
    ...
    it("omits VINCU_WORKTREE_PORT from teardown env when runtime metadata is missing", async () => {
      const vincuConfig = {
        worktree: {
          teardown: [
            'echo "port=${VINCU_WORKTREE_PORT-unset}" > "$VINCU_SOURCE_CHECKOUT_PATH/teardown-port.log"',
          ],
        },
      };
      writeFileSync(join(repoDir, "vincu.json"), JSON.stringify(vincuConfig));
    ...
    it("does not remove worktree when a teardown command fails", async () => {
      const vincuConfig = {
        worktree: {
          teardown: [
            'echo "started" > "$VINCU_SOURCE_CHECKOUT_PATH/teardown-start.log"',
      ...
      await expect(
        deleteVincuWorktree({ cwd: repoDir, worktreePath: created.worktreePath, vincuHome }),
