...
const expected = JSON.parse(process.env.VINCU_EXPECTED_ARGV_JSON);
const sliceFrom = process.env.VINCU_ARGV_SLICE_FROM ? Number(process.env.VINCU_ARGV_SLICE_FROM) : 2;
...
function makeFixture(): {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "vincu spawn regression "));
  tempDirs.push(root);
...
async function runFixture(params: {
  ...
  const child = spawnProcess(params.command, params.args, {
    env: {
      ...process.env,
      VINCU_EXPECTED_ARGV_JSON: JSON.stringify(["--config", JSON_ARG]),
    },
...
function makeLaunchFixture(ext: "exe" | "cmd" | "bat"): LaunchFixture {
  const root = mkdtempSync(path.join(tmpdir(), `vincu-launch-${ext}-`));
  tempDirs.push(root);

  // Unique base name so a globally installed binary cannot satisfy findExecutable.
  const command = `vincu-launch-fake-${path.basename(root)}`;
...
async function findAndLaunch(fixture: LaunchFixture): Promise<{
  ...
  return withPathPrepended(fixture.root, async () => {
    ...
    const child = spawnProcess(found, fixture.args, {
      env: {
        ...process.env,
        VINCU_EXPECTED_ARGV_JSON: fixture.expectedArgvJson,
        VINCU_ARGV_SLICE_FROM: String(fixture.sliceFrom),
      },
      stdio: ["ignore", "pipe", "pipe"],
    });
