...
function getPidFilePath(vincuHome: string): string {
  return join(vincuHome, "vincu.pid");
}
...
function createLockHeldError(lock: PidLockInfo): PidLockError {
  return new PidLockError(
    `Another Vincu daemon is already running (PID ${lock.pid}, started ${lock.startedAt})`,
    lock,
  );
}
  ...
  try {
    ...
    if (raceLock) {
      throw new PidLockError(
        `Another Vincu daemon is already running (PID ${raceLock.pid})`,
        raceLock,
      );
    }
...
export async function acquirePidLock(
  ...
  const pidPath = getPidFilePath(vincuHome);

  // Ensure vincuHome directory exists
  ...
  const lockInfo: PidLockInfo = {
    ...
    listen,
    heartbeat: true,
    ...(process.env.VINCU_DESKTOP_MANAGED === "1" ? { desktopManaged: true } : {}),
  };
...
export function startPidLockHeartbeat(
  ...
  const timer = setInterval(() => {
    ...
    refreshing = true;
    refreshPidLock(vincuHome, { ownerPid: options?.ownerPid })
      .catch((error) => {
        if (options?.onError) {
...
export async function getPidLockInfo(vincuHome: string): Promise<PidLockInfo | null> {
  const pidPath = getPidFilePath(vincuHome);
...
export async function isLocked(
  ...
  const info = await getPidLockInfo(vincuHome);
  if (!info) {
    return { locked: false };
  }
