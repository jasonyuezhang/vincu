...
import {
  ...
  handleVincuWorktreeArchiveRequest,
  handleVincuWorktreeListRequest,
  resolveGitCreateBaseBranch,
  runWorktreeSetupInBackground,
  handleCreateVincuWorktreeRequest,
...
function createWorkflowForRequestTest(options: {
  vincuHome: string;
  createVincuWorktree?: CreateVincuWorktreeFn;
  ...
  return async (input: Parameters<CreateVincuWorktreeFn>[0]) => {
    const createVincuWorktree =
      options.createVincuWorktree ?? createVincuWorktreeForTest({ vincuHome: options.vincuHome });
...
function createVincuWorktreeForTest(options: {
  vincuHome: string;
  events?: string[];
}): CreateVincuWorktreeFn {
  ...
  return (input, serviceOptions) => {
    return createVincuWorktreeService(input, {
...
describe("handleVincuWorktreeListRequest", () => {
  test("lists worktrees through the workspace git service", async () => {
    const emitted: SessionOutboundMessage[] = [];
    const workspaceGitService = {
      listWorktrees: vi.fn().mockResolvedValue([
        {
          path: "/tmp/vincu-home/worktrees/repo/feature",
    ...
    await handleVincuWorktreeListRequest(
      {
    ...
    expect(emitted).toContainEqual({
      type: "vincu_worktree_list_response",
      payload: {
        worktrees: [
          {
            worktreePath: "/tmp/vincu-home/worktrees/repo/feature",
            createdAt: "2026-04-12T00:00:00.000Z",
            branchName: "feature",
            head: "abc123",
          },
  ...
  test("agent setup continuation starts setup for the created agent timeline", async () => {
    ...
    try {
      const result = await createVincuWorktreeWorkflow(
        {
          vincuHome,
          createVincuWorktree: createVincuWorktreeForTest({ vincuHome }),
...
function createGitRepo(options?: { vincuConfig?: Record<string, unknown> }) {
  ...
  if (options?.vincuConfig) {
    writeFileSync(path.join(repoDir, "vincu.json"), JSON.stringify(options.vincuConfig, null, 2));
  }
...
describe("runWorktreeSetupInBackground", () => {
  ...
  test("runs setup from an exact workspace subdirectory", async () => {
    ...
    writeFileSync(
      path.join(sourceWorkspaceCwd, "vincu.json"),
      JSON.stringify({
        worktree: {
  ...
  test("archives the pending workspace and emits a failed snapshot when setup cannot start", async () => {
    ...
    writeFileSync(path.join(repoDir, "vincu.json"), "{ invalid json\n");
    ...
    expect(progressMessages[1]?.payload.error).toMatch(
      /Failed to parse vincu\.json at .*vincu\.json/,
    );
    expect(progressMessages[1]?.payload.detail.commands).toEqual([]);
    expect(snapshots.get(workspaceId)).toMatchObject({
      status: "failed",
      error: expect.stringMatching(/Failed to parse vincu\.json at .*vincu\.json/),
    });
    ...
    const snapshots = new Map([
      [
        "ws-feature-a",
        {
          ...
          detail: {
            type: "worktree_setup",
            worktreePath: "/repo/.vincu/worktrees/feature-a",
...
describe("handleCreateVincuWorktreeRequest", () => {
  const cleanupPaths: string[] = [];
  ...
  test("checks out the GitHub PR branch when githubPrNumber is supplied", async () => {
    ...
    await handleCreateVincuWorktreeRequest(
      {
        vincuHome,
        ...
        emit: (message) => emitted.push(message),
        sessionLogger: logger,
        createVincuWorktreeWorkflow: createWorkflowForRequestTest({ vincuHome }),
      },
      {
    ...
    const response = emitted.find(
      (
        message,
      ): message is Extract<SessionOutboundMessage, { type: "create_vincu_worktree_response" }> =>
        message.type === "create_vincu_worktree_response",
    );
    ...
    const result = await buildAgentSessionConfig(
      {
        ...
        createVincuWorktree: createVincuWorktreeForTest({
          vincuHome: path.join(tempDir, ".vincu"),
          events,
        }),
  ...
  test("buildAgentSessionConfig passes prompt and attachment context into worktree creation", async () => {
    const createVincuWorktree = vi.fn(async () => ({
      worktree: {
  ...
  test("createVincuWorktreeForTest forwards the default branch resolver for branch-off intents", async () => {
    const { tempDir, repoDir } = createGitRepo();
    cleanupPaths.push(tempDir);
    const vincuHome = path.join(tempDir, ".vincu");
    const resolveDefaultBranch = vi.fn(async () => "main");

    const result = await createVincuWorktreeForTest({ vincuHome })(
      {
        cwd: repoDir,
      ...
      await handleCreateVincuWorktreeRequest(
        {
          ...
          sessionLogger: createLogger(),
          emit: (message) => emitted.push(message),
          createVincuWorktreeWorkflow: createWorkflowForRequestTest({
            vincuHome,
            createVincuWorktree: createVincuWorktreeForTest({ vincuHome, events }),
          }),
  ...
  test("creates the worktree before emitting the response", async () => {
    ...
    try {
      await handleCreateVincuWorktreeRequest(
        {
          ...
          createVincuWorktreeWorkflow: createWorkflowForRequestTest({
            vincuHome,
            createVincuWorktree: async (input) => {
              const result = await createVincuWorktreeForTest({ vincuHome })(input);
      ...
      expect(emitted.map((message) => message.type).slice(0, 2)).toEqual([
        "create_vincu_worktree_response",
        "workspace_update",
      ]);
...
describe("handleVincuWorktreeArchiveRequest worktree scope", () => {
  const cleanupPaths: string[] = [];
  ...
  test("archives every active workspace on the directory and removes it", async () => {
    ...
    await handleVincuWorktreeArchiveRequest(
      {
        vincuHome,
        github: createGitHubServiceStub(),
        workspaceGitService: {
    ...
    expect(
      emitted.find((message) => message.type === "vincu_worktree_archive_response"),
    ).toMatchObject({
      payload: {
        success: true,
        error: null,
  ...
  test("ignores deleteWorktreeFromDisk:true and derives directory removal from remaining references", async () => {
    ...
    await handleVincuWorktreeArchiveRequest(deps, {
      type: "vincu_worktree_archive_request",
    ...
    expect(existsSync(sharedCwd)).toBe(false);
    expect(
      emitted.filter((message) => message.type === "vincu_worktree_archive_response"),
    ).toHaveLength(2);
  });
});
