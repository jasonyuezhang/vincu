...
interface SessionHandlerInternals {
  ...
  createVincuWorktree(params: unknown): Promise<unknown>;
...
const vincuWorktreeServiceMocks = vi.hoisted(() => ({
  createVincuWorktree: vi.fn(),
}));
...
vi.mock("./vincu-worktree-service.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("./vincu-worktree-service.js")>();
  return {
    ...actual,
    createVincuWorktree: vincuWorktreeServiceMocks.createVincuWorktree,
  };
});
...
function createSessionForTest(options: SessionForTestOptions = {}): Session {
  ...
  const sessionOptions: SessionOptions = {
    ...
    vincuHome: options.vincuHome ?? "/tmp/vincu-home",
...
describe("project command-center RPCs", () => {
  ...
  test("creates a directory and returns its normalized Project descriptor", async () => {
    const parentDirectory = realpathSync(mkdtempSync(join(tmpdir(), "vincu-project-session-")));
...
describe("workspace file access (behavior preservation)", () => {
  ...
  test("file upload round-trips bytes through binary frames", async () => {
    const vincuHome = makeDir("file-access-upload-");
    const messages: SessionOutboundMessage[] = [];
    const session = createSessionForTest({ messages, vincuHome });
    ...
    const session = createSessionForTest({
      ...
      daemonRuntimeConfig: {
        ...
        getRelayConfig: () => ({
          ...
          publicEndpoint: "relay.vincu.sh:443",
          useTls: true,
          publicUseTls: true,
        }),
      },
...
`;
  ...
  function writeConfig(repoRoot: string, config: unknown): void {
    writeFileSync(join(repoRoot, "vincu.json"), `${JSON.stringify(config)}\n`);
  }

  async function generateCommitPromptWithConfig(config: unknown): Promise<string> {
    const repoRoot = makeRoot();
    if (typeof config === "string") {
      writeFileSync(join(repoRoot, "vincu.json"), config);
  ...
  test.each([
    ["vincu.json missing", undefined],
    ["vincu.json exists but invalid JSON", "{ nope"],
    ["vincu.json valid but missing metadataGeneration", {}],
...
`;
  ...
  test("keeps the PR fallback when structured generation fails", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/getvincu/vincu/pull/9",
      number: 9,
    });
    const session = createSessionForTest({ workspaceGitService, messages });
    ...
    expect(checkoutGitMocks.createPullRequest).toHaveBeenCalledWith(
      "/tmp/request-worktree",
      {
        ...
        body: "Automated PR generated by Vincu.",
        base: "main",
      },
      expect.anything(),
    );
  ...
  test("forces workspace git and GitHub refresh after creating a pull request", async () => {
    ...
    checkoutGitMocks.createPullRequest.mockResolvedValue({
      url: "https://github.com/getvincu/vincu/pull/2",
      number: 2,
    });
    const session = createSessionForTest({ github, workspaceGitService, messages });
...
describe("session branch validation", () => {
  ...
  test("does not validate tags as branches", async () => {
    const tempDir = mkdtempSync(join(tmpdir(), "vincu-session-branch-validation-"));
...
describe("session stash list handling", () => {
  test("lists stashes through the workspace git service", async () => {
    ...
    const entries = [
      {
        index: 0,
        message: "vincu-auto-stash: feature",
        branch: "feature",
        isVincu: true,
      },
    ];
...
describe("session stash mutation handling", () => {
  test("forces a workspace git snapshot refresh after pushing a stash", async () => {
    ...
    expect(gitCommandMocks.runGitCommand).toHaveBeenCalledWith(
      ["stash", "push", "--include-untracked", "-m", "vincu-auto-stash: feature"],
...
describe("session vincu worktree creation handling", () => {
  test("forces workspace git refreshes for the source repo and created worktree", async () => {
    const workspaceGitService = { getSnapshot: vi.fn().mockResolvedValue({}) };
    const session = createSessionForTest({ workspaceGitService });
    vincuWorktreeServiceMocks.createVincuWorktree.mockResolvedValue({
      repoRoot: "/tmp/repo",
      worktree: {
        branchName: "feature/new-worktree",
        worktreePath: "/tmp/vincu/worktrees/new-worktree",
      },
      workspace: {
        workspaceId: "workspace-new-worktree",
        projectId: "project-repo",
        cwd: "/tmp/vincu/worktrees/new-worktree",
        kind: "worktree",
    ...
    await asSessionInternals(session).createVincuWorktree({
      cwd: "/tmp/repo",
    ...
    expect(workspaceGitService.getSnapshot).toHaveBeenCalledWith(
      "/tmp/vincu/worktrees/new-worktree",
      {
        force: true,
        reason: "create-worktree",
      },
...
describe("session workspace script handling", () => {
  test("passes the project slug and cached branch into workspace script spawning", async () => {
    ...
    const workspaceGitService = {
      ...
      getProjectSlug: vi.fn().mockResolvedValue("vincu"),
    };
...
describe("session pull request timeline handling", () => {
        ...
        items: [
          {
            ...
            number: 42,
            title: "Ship search",
            url: "https://github.com/getvincu/vincu/pull/42",
            state: "OPEN",
            body: null,
            labels: [],
            baseRefName: "main",
        ...
        items: [
          {
            ...
            url: "https://github.com/getvincu/vincu/pull/42#pullrequestreview-1",
  ...
  test.each([
    { prNumber: 0, repoOwner: "getvincu", repoName: "vincu" },
    { prNumber: -1, repoOwner: "getvincu", repoName: "vincu" },
    { prNumber: 42, repoOwner: "get vincu", repoName: "vincu" },
    { prNumber: 42, repoOwner: "getvincu/cli", repoName: "vincu" },
    { prNumber: 42, repoOwner: "get$vincu", repoName: "vincu" },
    { prNumber: 42, repoOwner: "getvincu", repoName: "pa seo" },
    { prNumber: 42, repoOwner: "getvincu", repoName: "vincu/app" },
    { prNumber: 42, repoOwner: "getvincu", repoName: "vincu!" },
  ...
  test("emits GitHub check details responses", async () => {
    ...
    const checkDetails: CheckDetails = {
      ...
      status: "completed",
      conclusion: "failure",
      url: "https://github.com/getvincu/vincu/actions/runs/456/job/789",
      detailsUrl: "https://github.com/getvincu/vincu/actions/runs/456/job/789",
