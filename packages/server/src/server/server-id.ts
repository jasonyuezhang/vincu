...
function getServerIdPath(vincuHome: string): string {
  return path.join(vincuHome, SERVER_ID_FILENAME);
}
...
/**
 * Stable daemon identifier scoped to a given $VINCU_HOME.
 *
 * - Persisted to `$VINCU_HOME/server-id`
 * - Can be overridden via `VINCU_SERVER_ID` (useful for tests)
 */
export function getOrCreateServerId(
  ...
  const serverIdPath = getServerIdPath(vincuHome);

  const envOverride =
    typeof env.VINCU_SERVER_ID === "string" && env.VINCU_SERVER_ID.trim().length > 0
      ? env.VINCU_SERVER_ID.trim()
      : null;
  ...
  if (envOverride) {
    ...
    if (!existsSync(serverIdPath)) {
      try {
        ...
        log?.info({ serverId: envOverride }, "Persisted VINCU_SERVER_ID override");
      } catch (error) {
        log?.warn({ error }, "Failed to persist VINCU_SERVER_ID override");
      }
    } else {
