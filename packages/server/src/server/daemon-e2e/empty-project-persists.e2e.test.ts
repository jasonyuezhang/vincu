...
test("project.add creates a project without creating a workspace", async () => {
  const previousSupervised = process.env.VINCU_SUPERVISED;
  process.env.VINCU_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "vincu-add-project-repo-")));
    const vincuHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "vincu-add-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(vincuHomeRoot);

    execSync("git init -b main", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.email 'test@getvincu.dev'", { cwd: repoRoot, stdio: "pipe" });
    execSync("git config user.name 'Vincu Test'", { cwd: repoRoot, stdio: "pipe" });
    ...
    restoreEnv("VINCU_SUPERVISED", previousSupervised);
  }
}, 30_000);
...
test("archiving the last workspace leaves the project parent with no workspaces", async () => {
  ...
  process.env.VINCU_SUPERVISED = "0";
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "vincu-empty-project-repo-")));
    const vincuHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "vincu-empty-project-home-")),
    );
    cleanupPaths.add(repoRoot);
    cleanupPaths.add(vincuHomeRoot);
    ...
    const projectsPath = path.join(vincuHome, "projects", "projects.json");
