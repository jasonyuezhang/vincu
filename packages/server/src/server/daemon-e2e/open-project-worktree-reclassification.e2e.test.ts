...
test("openProject preserves a worktree's exact-root project without rehoming it", async () => {
  ...
  try {
    const repoRoot = realpathSync(mkdtempSync(path.join(os.tmpdir(), "vincu-open-project-repo-")));
    const worktreeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "vincu-open-project-worktree-")),
    );
    const vincuHomeRoot = realpathSync(
      mkdtempSync(path.join(os.tmpdir(), "vincu-open-project-home-")),
    );
    ...
    const workspacesPath = path.join(vincuHome, "projects", "workspaces.json");
