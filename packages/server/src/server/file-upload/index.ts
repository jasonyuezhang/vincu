...
import { FileTransferOpcode, type FileTransferFrame } from "@getvincu/protocol/binary-frames/index";
...
export class FileUploadStore {
  ...
  beginUpload(request: FileUploadRequest): void {
    ...
    const uploadDir = join(this.vincuHome, "uploads", id);
  ...
  private async startWriting(upload: PendingUpload): Promise<void> {
    await mkdir(join(this.vincuHome, "uploads", upload.id), { recursive: true });
  ...
  private async removeUploadDirectory(upload: PendingUpload): Promise<void> {
    await rm(join(this.vincuHome, "uploads", upload.id), { recursive: true, force: true }).catch(
      () => undefined,
    );
  }
}
