...
import {
  markVincuWorktreeFirstAgentBranchAutoNameAttempted,
...
import type { FirstAgentContext } from "@getvincu/protocol/messages";
import { runWithGitCommandPriority } from "../utils/run-git-command.js";

export interface CreateVincuWorktreeInput extends CreateWorktreeCoreInput {
  projectId?: string;
  title?: string;
}

export interface CreateVincuWorktreeResult {
...
export type CreateVincuWorktreeFn = (
...
export interface CreateVincuWorktreeDeps extends CreateWorktreeCoreDeps {
  workspaceGitService: WorkspaceGitService;
  workspaceProvisioning: Pick<WorkspaceProvisioningService, "createWorkspaceForWorktree">;
}

export async function createVincuWorktree(
  input: CreateVincuWorktreeInput,
  deps: CreateVincuWorktreeDeps,
): Promise<CreateVincuWorktreeResult> {
  return runWithGitCommandPriority("high", () => createVincuWorktreeWithPriority(input, deps));
}

async function createVincuWorktreeWithPriority(
  input: CreateVincuWorktreeInput,
  deps: CreateVincuWorktreeDeps,
): Promise<CreateVincuWorktreeResult> {
  ...
  try {
    ...
    return rollbackCreatedVincuWorktree(
      {
...
export async function attemptFirstAgentBranchAutoName(options: {
  ...
  let metadata: ReturnType<typeof readVincuWorktreeMetadata>;
  try {
    metadata = readVincuWorktreeMetadata(options.cwd);
  } catch {
    return { attempted: false, renamed: false, branchName: null };
  }
  if (
  ...
  if ((await getCurrentBranchImpl(options.cwd)) !== placeholderBranchName) {
    markVincuWorktreeFirstAgentBranchAutoNameAttempted(options.cwd);
...
function maybeMarkFirstAgentBranchAutoNameEligible(options: {
  ...
  writeVincuWorktreeFirstAgentBranchAutoNameMetadata(createdWorktree.worktree.worktreePath, {
