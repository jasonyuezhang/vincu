...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "vincu-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.VINCU_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per VINCU_HOME", () => {
  ...
  it("respects and persists VINCU_SERVER_ID override", () => {
    process.env.VINCU_SERVER_ID = "test-daemon-id";
