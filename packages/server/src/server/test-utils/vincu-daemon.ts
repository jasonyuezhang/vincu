...
import {
  createVincuDaemon,
  type VincuDaemonConfig,
  type VincuOpenAIConfig,
  type VincuSpeechConfig,
} from "../bootstrap.js";
import type { AgentClient, AgentProvider } from "../agent/agent-sdk-types.js";
import { createTestAgentClients } from "./fake-agent-client.js";
import type { PushNotificationSender } from "../push/notifications.js";

interface TestVincuDaemonOptions {
  daemonVersion?: string;
  desktopManaged?: boolean;
  downloadTokenTtlMs?: number;
  corsAllowedOrigins?: string[];
  listen?: string;
  logger?: Parameters<typeof createVincuDaemon>[1];
  mcpEnabled?: boolean;
  mcpDebug?: boolean;
  isDev?: boolean;
  ...
  providerOverrides?: VincuDaemonConfig["providerOverrides"];
  vincuHomeRoot?: string;
  staticDir?: string;
  cleanup?: boolean;
  openai?: VincuOpenAIConfig;
  speech?: VincuSpeechConfig;
  voiceLlmProvider?: VincuDaemonConfig["voiceLlmProvider"];
  voiceLlmProviderExplicit?: boolean;
  voiceLlmModel?: string | null;
  dictationFinalTimeoutMs?: number;
  auth?: VincuDaemonConfig["auth"];
  pushNotificationSender?: PushNotificationSender;
  serviceProxy?: VincuDaemonConfig["serviceProxy"];
  webUi?: VincuDaemonConfig["webUi"];
  trustedProxies?: VincuDaemonConfig["trustedProxies"];
}

export interface TestVincuDaemon {
  config: VincuDaemonConfig;
  daemon: Awaited<ReturnType<typeof createVincuDaemon>>;
  port: number;
  vincuHome: string;
  staticDir: string;
  close: () => Promise<void>;
...
export async function createTestVincuDaemon(
  options: TestVincuDaemonOptions = {},
): Promise<TestVincuDaemon> {
  const maxAttempts = 8;
  let lastError: unknown;

  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    const { config, vincuHomeRoot, vincuHome, staticDir } = await prepareTestDaemonConfig(options);
    ...
    try {
      ...
      const close = async (): Promise<void> => {
        ...
        if (options.cleanup ?? true) {
          ...
          await Promise.all([
            rm(vincuHomeRoot, { recursive: true, force: true, maxRetries: 3, retryDelay: 100 }),
...
async function prepareTestDaemonConfig(
  ...
  const vincuHomeRoot =
    options.vincuHomeRoot ?? (await mkdtemp(path.join(os.tmpdir(), "vincu-home-")));
  const vincuHome = path.join(vincuHomeRoot, ".vincu");
  await mkdir(vincuHome, { recursive: true });
  const staticDir = options.staticDir ?? (await mkdtemp(path.join(os.tmpdir(), "vincu-static-")));
  ...
  const config: VincuDaemonConfig = {
    ...
    relayEndpoint: options.relayEndpoint ?? "relay.vincu.sh:443",
  ...
  return { config, vincuHomeRoot, vincuHome, staticDir };
}
