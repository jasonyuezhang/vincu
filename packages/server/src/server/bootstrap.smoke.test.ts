...
import { createVincuDaemon, parseListenString, type VincuDaemonConfig } from "./bootstrap.js";
...
describe("vincu daemon bootstrap", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  test("rolls back daemon listener when standalone service proxy startup fails", async () => {
    ...
    const vincuHomeRoot = await mkdtemp(path.join(os.tmpdir(), "vincu-standalone-rollback-"));
    const vincuHome = path.join(vincuHomeRoot, ".vincu");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "vincu-static-"));
    await mkdir(vincuHome, { recursive: true });
    const config: VincuDaemonConfig = {
      listen: "127.0.0.1:0",
      vincuHome,
      corsAllowedOrigins: [],
      hostnames: true,
    ...
    const daemon = await createVincuDaemon(config, pino({ level: "silent" }));
    ...
    try {
      ...
      await rm(vincuHomeRoot, { recursive: true, force: true });
  ...
  test("relay config changes during Hub enrollment reach the live runtime", async () => {
    const vincuHomeRoot = await mkdtemp(path.join(os.tmpdir(), "vincu-relay-startup-"));
    const vincuHome = path.join(vincuHomeRoot, ".vincu");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "vincu-static-"));
    await mkdir(vincuHome, { recursive: true });
    await writeFile(
      path.join(vincuHome, "hub-relationship.json"),
      `${JSON.stringify({
        version: 1,
        state: "pending",
        relationship: {
  ...
  test("rolls back already-open standalone listener when main daemon listen fails", async () => {
    ...
    const vincuHomeRoot = await mkdtemp(path.join(os.tmpdir(), "vincu-main-rollback-"));
    const vincuHome = path.join(vincuHomeRoot, ".vincu");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "vincu-static-"));
    await mkdir(vincuHome, { recursive: true });
    const config: VincuDaemonConfig = {
  ...
  test("starts when OpenAI speech provider is configured without credentials", async () => {
    const vincuHomeRoot = await mkdtemp(path.join(os.tmpdir(), "vincu-openai-config-"));
    const vincuHome = path.join(vincuHomeRoot, ".vincu");
    const staticDir = await mkdtemp(path.join(os.tmpdir(), "vincu-static-"));
    await mkdir(vincuHome, { recursive: true });
  ...
  test("does not block daemon start on local speech model downloads", async () => {
    ...
    const daemonHandle = await createTestVincuDaemon({
      speech: {
        ...
        local: {
          modelsDir: path.join(os.tmpdir(), `vincu-missing-models-${Date.now()}`),
          models: {
  ...
  test("rejects Windows absolute paths that are not named pipes", () => {
    ...
    expect(() => parseListenString(String.raw`D:\Users\foo\.vincu\daemon.sock`)).toThrow();
  ...
  test("parses Windows named pipes as managed IPC listen targets", () => {
    expect(parseListenString(String.raw`\\.\pipe\vincu-managed-test`)).toEqual({
      type: "pipe",
      path: String.raw`\\.\pipe\vincu-managed-test`,
    });
    expect(parseListenString(`pipe://${String.raw`\\.\pipe\vincu-managed-test`}`)).toEqual({
      type: "pipe",
      path: String.raw`\\.\pipe\vincu-managed-test`,
    });
  });
  ...
  test.skipIf(isPlatform("win32"))(
    ...
    async () => {
      const vincuHomeRoot = await mkdtemp(path.join(os.tmpdir(), "vincu-socket-relay-"));
      const vincuHome = path.join(vincuHomeRoot, ".vincu");
      const staticDir = await mkdtemp(path.join(os.tmpdir(), "vincu-static-"));
      const socketPath = path.join(vincuHomeRoot, "run", "vincu.sock");
      ...
      const daemon = await createVincuDaemon(config, logger);
      ...
      try {
        ...
        expect(pairing.url?.startsWith("https://app.vincu.sh/#offer=")).toBe(true);
      } finally {
        await daemon.stop().catch(() => undefined);
...
async function beginDaemonShutdownWithAgentClosing(): Promise<BlockedDaemonShutdown> {
  ...
  const agentCwd = await mkdtemp(path.join(os.tmpdir(), "vincu-shutdown-agent-"));
  ...
  return {
    ...
    async finish() {
      ...
      await Promise.all([
        rm(path.dirname(daemonHandle.vincuHome), { recursive: true, force: true }),
