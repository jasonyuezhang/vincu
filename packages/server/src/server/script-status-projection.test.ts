...
import { WorkspaceScriptPayloadSchema } from "@getvincu/protocol/messages";
import type { ScriptHealthState } from "./script-health-monitor.js";
import { WorkspaceScriptRuntimeStore } from "./workspace-script-runtime-store.js";
import { readVincuConfig } from "../utils/worktree.js";
import type { VincuConfig } from "@getvincu/protocol/vincu-config-schema";
import { createTestLogger } from "../test-utils/test-logger.js";

function createWorkspaceRepo(options?: {
  branchName?: string;
  vincuConfig?: Record<string, unknown>;
...
function buildPayloads(input: {
  ...
  const vincuConfig =
    input.vincuConfig !== undefined ? input.vincuConfig : loadConfig(input.workspaceDirectory);
...
function loadConfig(repoRoot: string): VincuConfig | null {
...
describe("script-status-projection", () => {
  ...
  it("readVincuConfig fails with configPath and error when vincu.json is malformed", () => {
    const workspace = createWorkspaceRepo();
    const configPath = path.join(workspace.repoDir, "vincu.json");
    writeFileSync(
      configPath,
      '{\n<<<<<<< HEAD\n  "scripts": {}\n=======\n  "scripts": {}\n>>>>>>> origin/main\n}\n',
    );
    ...
    try {
      const result = readVincuConfig(workspace.repoDir);
  ...
  it("buildWorkspaceScriptPayloads given vincuConfig=null still surfaces orphaned runtime scripts", () => {
