...
import { getForgeDefinitionOrNeutral } from "@getvincu/protocol/forge-manifest";
...
export class CheckoutSession {
  private static readonly VINCU_STASH_PREFIX = "vincu-auto-stash:";
  ...
  async handleStashSaveRequest(
    ...
    try {
      ...
      const message = branchLabel
        ? `${CheckoutSession.VINCU_STASH_PREFIX} ${branchLabel}`
        : `${CheckoutSession.VINCU_STASH_PREFIX} unnamed`;
  ...
  async handleStashListRequest(
    ...
    const { cwd, requestId } = msg;
    const vincuOnly = msg.vincuOnly !== false;
    try {
      const entries = await this.workspaceGitService.listStashes(cwd, { vincuOnly });
  ...
  async handleCheckoutMergeRequest(
    ...
    try {
      ...
      const mutatedCwd = await mergeToBase(
        ...
        { vincuHome: this.vincuHome, worktreesRoot: this.worktreesRoot },
      );
      await Promise.all([
