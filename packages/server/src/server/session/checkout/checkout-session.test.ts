...
describe("CheckoutSession", () => {
  ...
  describe("stash list", () => {
    it("returns stash entries scoped to vincu stashes by default", async () => {
      const listStashesCalls: Array<{ cwd: string; vincuOnly: boolean | undefined }> = [];
      const { checkout, emitted } = makeCheckoutSession({
        git: {
          listStashes: async (cwd, opts) => {
            listStashesCalls.push({ cwd, vincuOnly: opts?.vincuOnly });
            return [];
          },
        },
      });
      ...
      expect(listStashesCalls).toEqual([{ cwd: "/repo", vincuOnly: true }]);
      expect(emitted).toEqual([
        {
