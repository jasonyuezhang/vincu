...
interface CreateVincuWorktreeInBackgroundDependencies {
  vincuHome?: string;
  worktreesRoot?: string;
...
interface CreateVincuWorktreeWorkflowDependencies extends CreateVincuWorktreeInBackgroundDependencies {
  createVincuWorktree: (
    input: CreateVincuWorktreeInput,
    options?: {
      resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
...
export type CreateVincuWorktreeSetupContinuationInput =
  | { kind: "workspace" }
...
export type CreateVincuWorktreeWorkflowResult = CreateVincuWorktreeResult & {
  setupContinuation?: AgentWorktreeSetupContinuation;
};

export type CreateVincuWorktreeWorkflowFn = (
  input: CreateVincuWorktreeInput,
  options?: {
    resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    setupContinuation?: CreateVincuWorktreeSetupContinuationInput;
...
interface HandleCreateVincuWorktreeRequestDependencies {
  vincuHome?: string;
  worktreesRoot?: string;
...
export async function buildAgentSessionConfig(
  ...
  if (normalized.createWorktree) {
    ...
    const createdWorktree = await dependencies.createVincuWorktree(
      {
        cwd,
...
export async function handleVincuWorktreeListRequest(
  dependencies: {
    emit: EmitSessionMessage;
    vincuHome?: string;
    workspaceGitService: WorkspaceGitService;
  ...
  try {
    const worktrees = await listVincuWorktreesCommand(
...
export async function handleVincuWorktreeArchiveRequest(
  dependencies: Omit<
    ArchiveDependencies,
  ...
  try {
    ...
    if (!result.ok) {
      dependencies.emit({
        type: "vincu_worktree_archive_response",
        payload: {
          success: false,
...
export async function handleCreateVincuWorktreeRequest(
  dependencies: HandleCreateVincuWorktreeRequestDependencies,
  request: Extract<SessionInboundMessage, { type: "create_vincu_worktree_request" }>,
): Promise<void> {
  try {
    const commandResult = await createVincuWorktreeCommand(
      {
        vincuHome: dependencies.vincuHome,
        worktreesRoot: dependencies.worktreesRoot,
        createVincuWorktreeWorkflow: dependencies.createVincuWorktreeWorkflow,
      },
      {
        cwd: request.cwd,
...
export async function createVincuWorktreeWorkflow(
  dependencies: CreateVincuWorktreeWorkflowDependencies,
  input: CreateVincuWorktreeInput,
  options?: {
    resolveDefaultBranch?: (repoRoot: string) => Promise<string>;
    setupContinuation?: CreateVincuWorktreeSetupContinuationInput;
...
export async function runWorktreeSetupInBackground(
  dependencies: CreateVincuWorktreeInBackgroundDependencies,
  options: {
    requestCwd: string;
    repoRoot: string;
    workspaceId: string;
