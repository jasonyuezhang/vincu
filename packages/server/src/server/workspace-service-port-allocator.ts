...
export interface AllocateWorkspaceServicePortOptions {
  allocation: VincuServicePortAllocation | undefined;
  cwd: string;
  scriptName: string;
  workspaceId: string;
  ...
  try {
    result = await execCommand(
      ...
      [options.scriptName, options.workspaceId, options.branchName ?? "", options.cwd],
      {
        cwd: options.cwd,
        envOverlay: {
          VINCU_SCRIPTNAME: options.scriptName,
          VINCU_WORKSPACE_ID: options.workspaceId,
          VINCU_BRANCH_NAME: options.branchName ?? "",
          VINCU_WORKTREE_PATH: options.cwd,
        },
