...
describe("resolveSpeechConfig", () => {
  test("resolves local-first defaults without env overrides", () => {
    const vincuHome = "/tmp/vincu-home";
    ...
    expect(result.speech.local).toEqual({
      modelsDir: path.join(vincuHome, "models", "local-speech"),
      models: {
  ...
  test("resolves feature-scoped local speech settings", () => {
    ...
    const env = {
      VINCU_DICTATION_LOCAL_STT_MODEL: "parakeet-tdt-0.6b-v2-int8",
      VINCU_VOICE_LOCAL_STT_MODEL: "parakeet-tdt-0.6b-v2-int8",
      VINCU_VOICE_LOCAL_TTS_MODEL: "kokoro-en-v0_19",
      VINCU_VOICE_LOCAL_TTS_SPEAKER_ID: "5",
      VINCU_VOICE_LOCAL_TTS_SPEED: "1.35",
      VINCU_DICTATION_LANGUAGE: "es",
      VINCU_VOICE_LANGUAGE: "pt",
      VINCU_LOCAL_MODELS_DIR: "/tmp/models",
      OPENAI_API_KEY: "env-key",
      VINCU_VOICE_STT_PROVIDER: "openai",
      VINCU_DICTATION_STT_PROVIDER: "local",
      VINCU_VOICE_TTS_PROVIDER: "local",
