...
export type PersistedWorkspacePlacement = Pick<
  ...
  | "worktreeRoot"
  | "baseBranch"
  | "isVincuOwnedWorktree"
  | "mainRepoRoot"
>;

export type MutableWorkspacePlacement = Pick<
  PersistedWorkspaceRecord,
  "kind" | "branch" | "worktreeRoot" | "isVincuOwnedWorktree" | "mainRepoRoot"
>;
...
export function initialWorkspacePlacement(
  ...
  return {
    ...
    baseBranch: null,
    isVincuOwnedWorktree: input.checkout.isGit && input.checkout.isVincuOwnedWorktree,
...
export function reconcileWorkspacePlacement(input: {
  ...
  if (input.workspace.isVincuOwnedWorktree !== observed.isVincuOwnedWorktree)
    fields.isVincuOwnedWorktree = observed.isVincuOwnedWorktree;
...
export function checkoutFromPersistedWorkspacePlacement(input: {
  ...
  if (workspace.isVincuOwnedWorktree && workspace.mainRepoRoot) {
    return {
      ...checkout,
      isGit: true,
      isVincuOwnedWorktree: true,
...
export function checkoutLiteFromGitSnapshot(
  ...
  if (git.isVincuOwnedWorktree && git.mainRepoRoot) {
    return {
      cwd,
      isGit: true,
