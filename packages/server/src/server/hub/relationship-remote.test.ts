...
afterEach(async () => {
  ...
  await Promise.all(openVincuHomes.splice(0).map((home) => rm(home, { recursive: true })));
});
...
async function connectController(
  ...
  const vincuHome = await mkdtemp(path.join(tmpdir(), "vincu-hub-socket-"));
  openVincuHomes.push(vincuHome);
