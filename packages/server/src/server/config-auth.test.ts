...
async function createVincuHome(config: unknown): Promise<string> {
  const root = await mkdtemp(path.join(os.tmpdir(), "vincu-config-auth-"));
  roots.push(root);
...
describe("daemon auth config", () => {
  ...
  test("loads optional auth password hash from config.json", async () => {
    const vincuHome = await createVincuHome({
      version: 1,
      daemon: {
  ...
  test("lets VINCU_PASSWORD override config.json auth password hash", async () => {
    const vincuHome = await createVincuHome({
      version: 1,
      daemon: {
        auth: { password: CONFIG_PASSWORD_HASH },
