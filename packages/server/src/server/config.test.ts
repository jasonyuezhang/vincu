...
describe("server config", () => {
  ...
  test("records when the daemon is managed by Vincu Desktop", async () => {
    const vincuHome = await mkdtemp(path.join(os.tmpdir(), "vincu-config-desktop-managed-"));
    roots.push(vincuHome);

    const desktopConfig = loadConfig(vincuHome, {
      env: { VINCU_DESKTOP_MANAGED: "1" },
    });
    const standaloneConfig = loadConfig(vincuHome, { env: {} });
  ...
  test("resolves bundled web UI path from globally installed compiled modules", async () => {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "vincu-config-compiled-"));
  ...
  test("resolves packaged desktop web UI path from resources app-dist", async () => {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "vincu-config-packaged-"));
    roots.push(packageRoot);
