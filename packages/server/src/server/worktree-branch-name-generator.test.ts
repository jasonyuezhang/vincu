...
describe("generateBranchNameFromFirstAgentContext", () => {
  ...
  test("keeps the branch slug validator fallback when instructions are present", async () => {
    const repoRoot = createTempDir("vincu-branch-config-");
    const worktreeRoot = createTempDir("vincu-branch-worktree-");
    mkdirSync(path.join(worktreeRoot, ".git"));
    writeVincuWorktreeMetadata(worktreeRoot, { baseRefName: "main" });
    writeVincuWorktreeFirstAgentBranchAutoNameMetadata(worktreeRoot, {
...
async function generateBranchPromptWithConfig(config: unknown): Promise<{ prompt: string }> {
  ...
  if (typeof config === "string") {
    writeFileSync(path.join(repoRoot, "vincu.json"), config);
...
function writeConfig(repoRoot: string, config: unknown): void {
  writeFileSync(path.join(repoRoot, "vincu.json"), `${JSON.stringify(config)}\n`);
}
