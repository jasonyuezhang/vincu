...
// Stub overrides so tests can opt into the old "all builtins registered" behavior.
const ALL_BUILTINS = {
  claude: {},
  codex: {},
  cursor: {},
...
describe("ProviderSnapshotManager public surface", () => {
  test("listRegisteredProviderIds is empty without provider overrides", () => {
    const manager = new ProviderSnapshotManager({ logger: createTestLogger() });
    try {
      expect(manager.listRegisteredProviderIds()).toEqual([]);
    } finally {
      manager.destroy();
    }
  });

  test("listRegisteredProviderIds includes builtins when overrides exist", () => {
    const manager = new ProviderSnapshotManager({
      logger: createTestLogger(),
      providerOverrides: { ...ALL_BUILTINS },
    });
    try {
  ...
  test("hasProvider reflects providerOverrides, not bare builtins", () => {
    const manager = new ProviderSnapshotManager({
      logger: createTestLogger(),
      providerOverrides: {
    ...
    try {
      expect(manager.hasProvider("claude")).toBe(false);
  ...
  test("VINCU_PROVIDER_REFRESH_TIMEOUT_MS env var is honored when no option is given", async () => {
    vi.stubEnv("VINCU_PROVIDER_REFRESH_TIMEOUT_MS", "1");
    const isAvailable = vi.fn(() => new Promise<boolean>(() => {}));
    const manager = new ProviderSnapshotManager({
      logger: createTestLogger(),
      providerOverrides: {
  ...
  test("VINCU_PROVIDER_REFRESH_TIMEOUT_MS env var is ignored when option is provided", async () => {
    vi.stubEnv("VINCU_PROVIDER_REFRESH_TIMEOUT_MS", "1");
    const isAvailable = vi.fn(() => new Promise<boolean>(() => {}));
    const manager = new ProviderSnapshotManager({
      logger: createTestLogger(),
  ...
  test("getProviderDiagnostic reports a stuck catalog refresh inside the diagnostic", async () => {
    await withEnv("VINCU_ENABLE_MOCK_SLOW", "true", async () => {
      vi.useFakeTimers();
      const manager = new ProviderSnapshotManager({
        logger: createTestLogger(),
        isDev: true,
