...
import { everyMsToFiveFieldCron } from "@getvincu/protocol/schedule/cadence";
import { expandUserPath, isSameOrDescendantPath, resolvePathFromBase } from "../../path-utils.js";
import type { TerminalManager } from "../../../terminal/terminal-manager.js";
import type { CreateVincuWorktreeWorkflowFn } from "../../worktree-session.js";
...
import {
  ...
  type CreateVincuWorktreeCommandInput,
  createVincuWorktreeCommand,
...
export interface VincuToolHostDependencies {
  ...
  createVincuWorktree?: CreateVincuWorktreeWorkflowFn;
...
type WorkspaceWorktreeTarget = Pick<
  CreateVincuWorktreeCommandInput,
...
export function createVincuToolCatalog(options: VincuToolHostDependencies): VincuToolCatalog {
  const {
    agentManager,
    agentStorage,
    terminalManager,
  ...
  const childLogger = logger.child({ module: "agent", component: "vincu-tool-catalog" });
  const callerContext = callerAgentId ? (resolveCallerContext?.(callerAgentId) ?? null) : null;

  const parseToolInput = async (tool: VincuToolDefinition, input: unknown): Promise<unknown> => {
  ...
  const tools = new Map<string, VincuToolDefinition>();
  const registerTool = (
    name: string,
    ...
    handler: (input: any, context: VincuToolExecutionContext) => Promise<VincuToolResult>,
  ) => {
    tools.set(name, {
      name,
      title: config.title,
      ...
      handler: handler as VincuToolDefinition["handler"],
    });
  };
  const toCatalog = (): VincuToolCatalog => ({
    tools,
    ...
    async executeTool(
      ...
      if (!tool) {
        throw new Error(`Vincu tool not found: ${name}`);
      }
    ...
    z
      .object({
        ...
        worktreeSlug: z
          ...
          .min(1)
          .optional()
          .describe("Optional worktree slug/path label. Omit to let Vincu generate one."),
        branchName: z
          .string()
          .min(1)
          .optional()
      ...
      .strict()
      .describe("Create a new branch in a new Vincu worktree."),
    z
      .object({
      ...
      .strict()
      .describe("Check out an existing branch in a new Vincu worktree."),
    z
      .object({
      ...
      .strict()
      .describe("Check out a GitHub pull request in a new Vincu worktree."),
  ]);
  ...
  registerTool(
    "create_workspace",
    {
      ...
      description:
        "Create a workspace using an existing local checkout or a new Vincu-managed worktree.",
      inputSchema: {
        isolation: z.enum(["local", "worktree"]),
        path: z
          .string()
    ...
    async ({
      ...
      if (isolation === "local") {
        ...
        const result = await createVincuWorktreeCommand(
          {
            vincuHome: options.vincuHome,
            worktreesRoot: options.worktreesRoot,
            createVincuWorktreeWorkflow: options.createVincuWorktree,
          },
          {
            cwd,
            ...(projectId ? { projectId } : {}),
    ...
    async (args: unknown) => {
      ...
      const {
      ...
      } = await createAgentCommand(
        {
          ...
          terminalManager,
          providerSnapshotManager,
          createVincuWorktree: options.createVincuWorktree,
  ...
  registerTool(
    "start_workspace_script",
    {
      ...
      description:
        "Start one configured workspace script through Vincu's managed workspace-script launcher.",
      inputSchema: {
        workspaceId: z.string().describe("Workspace ID containing the configured script."),
        scriptName: z.string().min(1).describe("Configured vincu.json script name to start."),
      },
      outputSchema: {
  ...
  registerTool(
    "stop_workspace_script",
    {
      ...
      inputSchema: {
        ...
        scriptName: z.string().min(1).describe("Configured vincu.json script name to stop."),
      },
      outputSchema: {
        script: WorkspaceScriptPayloadSchema,
      },
...
function archiveWorktreeDependencies(
  options: VincuToolHostDependencies,
  ...
  return {
    vincuHome: options.vincuHome,
    vincuWorktreesBaseRoot: options.worktreesRoot,
