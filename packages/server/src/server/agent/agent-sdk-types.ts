...
import type { AgentProviderNotice } from "@getvincu/protocol/agent-types";
...
export interface AgentCapabilityFlags {
  ...
  supportsNativeVincuTools?: boolean;
...
export interface AgentLaunchContext {
  ...
  env?: Record<string, string>;
  /**
   * Runtime-only internal Vincu tools. This must never be persisted into
...
export interface AgentClient {
   ...
   * Called when Vincu archives an agent so the provider's own UI reflects the same state.
   */
  archiveNativeSession?(handle: AgentPersistenceHandle): Promise<void>;
  /**
   * Unarchive a durable native session in the provider.
   * Called before Vincu clears its archived flag so provider resume can succeed.
   */
