...
describe("codex tool-call mapper", () => {
  ...
  it("normalizes codex vincu speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
  ...
  it("normalizes codex vincu_voice.speak mcp calls and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallFromThreadItem({
        type: "mcpToolCall",
        ...
        server: "vincu_voice",
        tool: "speak",
        arguments: { text: "Voice response from Codex via vincu_voice." },
        result: { ok: true },
      }),
    );
    ...
    expect(item.detail).toEqual({
      type: "unknown",
      input: "Voice response from Codex via vincu_voice.",
      output: null,
    });
  });

  it("normalizes codex vincu speak tool-call names and extracts spoken text", () => {
    const item = expectMapped(
      mapCodexToolCallEnvelope({
