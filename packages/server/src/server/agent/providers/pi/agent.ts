...
const VINCU_PI_TREE_EXTENSION_COMMAND = "vincu_tree";
const VINCU_PI_CAPTURE_EXTENSION_COMMAND = "vincu_capture_entries";
const VINCU_PI_ENTRY_CAPTURE_MARKER = "VINCU_ENTRY_CAPTURE";
const VINCU_PI_SUBMITTED_USER_ENTRY_MARKER = "VINCU_SUBMITTED_USER_ENTRY";
const VINCU_PI_COMMAND_RESULT_MARKER = "VINCU_COMMAND_RESULT";
...
function buildResumeStartInput(input: {
  ...
  vincuExtension: PiTempFile | null;
}): PiStartSessionInput {
  return {
    ...
    extensionPaths: input.vincuExtension ? [input.vincuExtension.path] : undefined,
  };
}
...
function createPiMcpConfigFile(
  ...
  const dir = mkdtempSync(join(tmpdir(), "vincu-pi-mcp-"));
...
function createPiVincuExtensionFile(systemPrompt?: string): PiTempFile {
  const dir = mkdtempSync(join(tmpdir(), "vincu-pi-extension-"));
  const filePath = join(dir, "vincu-integration.mjs");
  writeFileSync(
    filePath,
    `
	...
	function emitCommandResult(ctx, requestId, result) {
	  ctx.ui.notify(
	    "${VINCU_PI_COMMAND_RESULT_MARKER} " + JSON.stringify({ requestId, ...result }),
	    result.ok ? "info" : "error",
	  );
	}

	export default function vincuIntegration(pi) {
	  ...
	  function emitSubmittedUserEntries(ctx) {
	    ...
	    for (let index = 0; index < submittedUserMessages.length; index += 1) {
	      ...
	      index -= 1;
	      ctx.ui.notify(
	        "${VINCU_PI_SUBMITTED_USER_ENTRY_MARKER} " +
	  ...
	  pi.registerCommand("${VINCU_PI_CAPTURE_EXTENSION_COMMAND}", {
	    description: "Internal Vincu entry capture bridge",
	  ...
	  pi.registerCommand("${VINCU_PI_TREE_EXTENSION_COMMAND}", {
	    description: "Internal Vincu tree navigation bridge",
	    handler: async (args, ctx) => {
	      const payload = decodePayload(args.trim());
	      try {
...
export class PiRpcAgentSession implements AgentSession {
  ...
  private async runPiTreeExtensionCommand(targetId: string): Promise<unknown> {
    ...
    await this.runtimeSession.prompt(`/${VINCU_PI_TREE_EXTENSION_COMMAND} ${payload}`);
  ...
  private async requestEntryCapture(reason: string): Promise<void> {
    ...
    await this.runtimeSession.prompt(`/${VINCU_PI_CAPTURE_EXTENSION_COMMAND} ${payload}`);
    await resultPromise;
  }
  ...
  private handleSubmittedUserEntryMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, VINCU_PI_SUBMITTED_USER_ENTRY_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleEntryCaptureMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, VINCU_PI_ENTRY_CAPTURE_MARKER);
    if (!payload) {
      return false;
    }
  ...
  private handleCommandResultMarker(message: string): boolean {
    const payload = parseExtensionMarkerPayload(message, VINCU_PI_COMMAND_RESULT_MARKER);
    if (!payload) {
      return false;
    }
    if (typeof payload.requestId !== "string") {
...
export class PiRpcAgentClient implements AgentClient {
  ...
  async createSession(
    ...
    const vincuExtension = createPiVincuExtensionFile(
    ...
    try {
      runtimeSession = await this.runtime.startSession({
        ...
        extensionPaths: vincuExtension ? [vincuExtension.path] : undefined,
      });
    } catch (error) {
      mcpConfig?.cleanup();
      vincuExtension?.cleanup();
      throw error;
    }
    try {
      return new PiRpcAgentSession({
        ...
        cleanup: combineCleanup([mcpConfig?.cleanup, vincuExtension?.cleanup]),
