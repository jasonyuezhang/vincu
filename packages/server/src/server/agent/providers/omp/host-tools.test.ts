...
import type { VincuToolCatalog, VincuToolDefinition, VincuToolResult } from "../../tools/types.js";
import {
...
function createCatalog(tools: VincuToolDefinition[]): VincuToolCatalog {
...
class OmpHostToolHarness {
  ...
  private resolveControlledResult: ((result: VincuToolResult) => void) | null = null;
  ...
  static async withTools(tools: VincuToolDefinition[]): Promise<OmpHostToolHarness> {
  ...
  static async cancellable(): Promise<OmpHostToolHarness> {
    ...
    const tool: VincuToolDefinition = {
      name: "wait_for_agent",
      description: "Wait for a Vincu agent.",
      handler: async (_input, context) => {
        harness.controlledSignal = context.signal ?? null;
        harness.resolveControlledStart?.();
        return await new Promise<VincuToolResult>((resolve) => {
  ...
  completeControlledCall(result: VincuToolResult): void {
  ...
  private routerInput() {
    return { runtimeSession: this.runtimeSession, vincuTools: this.catalog, logger: this.logger };
  }
}

describe("OMP host tools", () => {
  test("marks every caller-scoped Vincu tool essential for direct invocation", () => {
    const catalog = createCatalog([
      {
        name: "create_agent",
        title: "Create agent",
        description: "Create a Vincu agent.",
