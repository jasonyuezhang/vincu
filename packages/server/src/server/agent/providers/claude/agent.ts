...
import { resolveClaudeConfigDirFromRuntime } from "../../../provider-accounts/resolve-home.js";
...
export function normalizeClaudeAskUserQuestionRequestInput(
  ...
  // model-supplied option. Vincu's shared question UI uses allowOther for that
...
export function normalizeClaudeAskUserQuestionUpdatedInput(
  ...
  // Vincu's shared question UI serializes answers by question header, but Claude's
...
export class ClaudeAgentClient implements AgentClient {
  ...
  async listImportableSessions(
    ...
    const configDir = resolveClaudeConfigDirFromRuntime(this.runtimeSettings);
...
class ClaudeAgentSession implements AgentSession {
   ...
   * — and the level Vincu requests is not necessarily the level that runs, because a model that
  ...
  private mapBlocksToTimeline(
    ...
    const items: AgentTimelineItem[] = [];
    // User SDK entries can arrive as multiple text blocks, but Vincu treats them as one message.
