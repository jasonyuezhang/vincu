...
test("logs redacted query summary and never leaks sentinel secrets", async () => {
  const envSecret = "VINCU_ENV_SENTINEL_SECRET";
  const runtimeSecret = "VINCU_RUNTIME_SENTINEL_SECRET";
  const systemSecret = "VINCU_SYSTEM_PROMPT_SENTINEL_SECRET";
  const previousEnv = process.env.VINCU_TEST_SENTINEL_SECRET;
  process.env.VINCU_TEST_SENTINEL_SECRET = envSecret;
  ...
  const client = new ClaudeAgentClient({
    ...
    runtimeSettings: {
      env: {
        VINCU_RUNTIME_SENTINEL_SECRET: runtimeSecret,
      },
    },
    resolveBinary: async () => "/test/claude/bin",
  });
  ...
  try {
    ...
    if (previousEnv === undefined) {
      delete process.env.VINCU_TEST_SENTINEL_SECRET;
    } else {
      process.env.VINCU_TEST_SENTINEL_SECRET = previousEnv;
    }
  }
});
