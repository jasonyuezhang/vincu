...
describe("STTManager", () => {
  ...
  it("uses VINCU_VOICE_LANGUAGE over VINCU_DICTATION_LANGUAGE", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
        VINCU_VOICE_LANGUAGE: "pt",
        VINCU_DICTATION_LANGUAGE: "es",
  ...
  it("uses VINCU_DICTATION_LANGUAGE when VINCU_VOICE_LANGUAGE is unset", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.VINCU_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.VINCU_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.VINCU_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.VINCU_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
