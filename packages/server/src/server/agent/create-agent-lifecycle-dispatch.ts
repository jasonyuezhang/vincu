...
interface CreateAgentLifecycleDispatchDependencies {
  ...
  createVincuWorktreeWorkflow: CreateVincuWorktreeWorkflowFn;
...
type AutoArchiveTarget =
  | { kind: "agent-only" }
  | { kind: "created-worktree"; result: CreateVincuWorktreeWorkflowResult };
...
export class CreateAgentLifecycleDispatch {
  ...
  }): Promise<CreateVincuWorktreeWorkflowResult | null> {
  ...
  private async createWorktreeForTarget(
    ...
    const baseInput = {
      ...
      firstAgentContext,
      runSetup: false,
      vincuHome: this.dependencies.vincuHome,
      worktreesRoot: this.dependencies.worktreesRoot,
    } as const;

    switch (target.mode) {
      case "branch-off":
        return this.dependencies.createVincuWorktreeWorkflow(
          {
            ...baseInput,
  ...
  private async archiveAutoCreatedWorktree(options: {
    ...
    const ownership = await isVincuOwnedWorktreeCwd(worktreePath, {
      vincuHome: this.dependencies.vincuHome,
      worktreesRoot: this.dependencies.worktreesRoot,
    });
    if (!ownership.allowed) {
      throw new Error("Auto-created worktree is not a Vincu-owned worktree");
    }

    await archiveByScope(
      {
        vincuHome: this.dependencies.vincuHome,
        vincuWorktreesBaseRoot: this.dependencies.worktreesRoot,
