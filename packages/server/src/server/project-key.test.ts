...
describe("deriveProjectKey", () => {
  ...
  test.each([
    ...
    "git@github.com:getvincu/vincu.git",
  ])("normalizes common remote form %s", (remoteUrl) => {
    expect(derive(remoteUrl)).toBe("remote:github.com/getvincu/vincu");
  });

  test("normalizes GitHub casing", () => {
    expect(derive("git@github.com:GetVincu/Vincu.git")).toBe("remote:github.com/getvincu/vincu");
  });
  ...
  test("groups an SSH remote whether or not its default port is spelled out", () => {
    expect(derive("ssh://git@github.com:22/getvincu/vincu.git")).toBe(
      derive("ssh://git@github.com/getvincu/vincu.git"),
    );
  });
  ...
  test("keeps a repository root distinct from one of its subprojects", () => {
    ...
    const remoteUrl = "git@github.com:getvincu/vincu.git";

    expect(
      deriveProjectKey({ rootPath: worktreeRoot, remoteUrl, worktreeRoot, mainRepoRoot: null }),
    ).toBe("remote:github.com/getvincu/vincu");
    expect(
      deriveProjectKey({
    ...
    ).toBe("remote:github.com/getvincu/vincu#subdir:packages/app");
  });
