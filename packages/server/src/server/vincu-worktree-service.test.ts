...
import {
  ...
  createVincuWorktree,
  type CreateVincuWorktreeDeps,
} from "./vincu-worktree-service.js";
import { readVincuWorktreeMetadata } from "../utils/worktree-metadata.js";
import { createWorktree, getVincuWorktreesRoot } from "../utils/worktree.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createVincuWorktree(
    {
      cwd: repoDir,
...
test("seeds an uncommitted exact-project config into the mapped worktree directory", async () => {
  ...
  writeFileSync(path.join(sourceDir, "vincu.json"), config);
  ...
  expect(readFileSync(path.join(result.workspace.cwd, "vincu.json"), "utf8")).toBe(config);
  expect(existsSync(path.join(result.worktree.worktreePath, "vincu.json"))).toBe(false);
});
...
test("removes a new worktree when its ref does not contain the selected project directory", async () => {
  ...
  const worktreePath = path.join(
    await getVincuWorktreesRoot(repoDir, vincuHome),
    "missing-subproject",
  );
...
test("maps a nested cwd from an existing Vincu worktree into the next worktree", async () => {
  const { repoDir, tempDir } = createGitRepo();
  cleanupPaths.push(tempDir);
  const vincuHome = path.join(tempDir, ".vincu");
  ...
  commitAll(repoDir, "add subproject");
  const deps = createDeps();
  const source = await createVincuWorktree(
    {
      cwd: repoDir,
  ...
  const created = await createVincuWorktree(
    {
      cwd: sourceCwd,
...
test("rejects source checkout planning before creating a worktree", async () => {
  ...
  expect(existsSync(path.join(vincuHome, "worktrees"))).toBe(false);
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createVincuWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createVincuWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
test("renames an eligible unnamed branch-off worktree once on first agent context", async () => {
  ...
  expect(readVincuWorktreeMetadata(created.worktree.worktreePath)).toMatchObject({
    version: 2,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "vincu-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
// Worktree restore (Unit 3): recreate a deleted Vincu-owned worktree from its
...
// repoRoot=null, so deleteVincuWorktree's `git worktree remove`/`prune` is
...
interface TestDeps extends CreateVincuWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isVincuOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
