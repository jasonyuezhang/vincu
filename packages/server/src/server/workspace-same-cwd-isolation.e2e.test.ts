...
function seedSameCwdWorkspaces(): { vincuHomeRoot: string; cwd: string } {
  const vincuHomeRoot = mkdtempSync(path.join(tmpdir(), "vincu-same-cwd-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "vincu-same-cwd-dir-"));
  const projectsDir = path.join(vincuHomeRoot, ".vincu", "projects");
  ...
  return { vincuHomeRoot, cwd };
}

async function seedWorkspaceWithLegacyAgent(): Promise<{ vincuHomeRoot: string; cwd: string }> {
  const vincuHomeRoot = mkdtempSync(path.join(tmpdir(), "vincu-legacy-agent-home-"));
  const cwd = mkdtempSync(path.join(tmpdir(), "vincu-legacy-agent-dir-"));
  ...
  const agentStorage = new AgentStorage(path.join(vincuHome, "agents"), createTestLogger());
...
test("daemon bootstrap migrates cwd-only legacy agents before same-cwd workspaces are added", async () => {
  const { vincuHomeRoot, cwd } = await seedWorkspaceWithLegacyAgent();
  const daemon = await createTestVincuDaemon({ vincuHomeRoot });
...
test("workspace.create directory source with firstAgentContext generates a daemon-visible workspace title", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "vincu-named-local-dir-"));
...
test("local workspace auto-title does not broadcast provider snapshot warm-up to clients", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "vincu-title-snapshot-storm-"));
...
test("create_agent_request with workspaceId does not retitle an existing workspace", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "vincu-agent-submit-title-"));
  const daemon = await createTestVincuDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("creating another same-cwd local workspace keeps running status on the owning workspace only", async () => {
  const cwd = mkdtempSync(path.join(tmpdir(), "vincu-running-same-cwd-create-"));
  const daemon = await createTestVincuDaemon({
    agentClients: { mock: new MockLoadTestAgentClient() },
  });
  const client = new DaemonClient({
...
test("two workspaces sharing one cwd compute agent status per workspaceId", async () => {
  const { vincuHomeRoot, cwd } = seedSameCwdWorkspaces();
  const daemon = await createTestVincuDaemon({ vincuHomeRoot });
  const client = new DaemonClient({
    url: `ws://127.0.0.1:${daemon.port}/ws`,
    appVersion: "0.1.82",
