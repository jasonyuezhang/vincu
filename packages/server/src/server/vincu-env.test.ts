...
import {
  ...
  createVincuInternalEnv,
  resolveVincuNodeEnv,
} from "./vincu-env.js";

describe("vincu env contract", () => {
  const ELECTRON_RUN_AS_NODE = "ELECTRON_RUN_AS_NODE";
  const VINCU_NODE_ENV = "VINCU_NODE_ENV";
  const baseEnv = {
    [ELECTRON_RUN_AS_NODE]: "1",
    ELECTRON_NO_ATTACH_CONSOLE: "1",
    NODE_ENV: "development",
    PATH: "/usr/bin",
    VINCU_AGENT_ID: "agent-123",
  ...
  test("builds internal daemon child env by preserving pass-through and control vars", () => {
    const env = createVincuInternalEnv(baseEnv);
  ...
  test("builds external process env by scrubbing runtime control vars after overlays", () => {
    ...
    expect(env.VINCU_AGENT_ID).toBe("agent-123");
  ...
  test("builds external command env without process.execPath special-casing", () => {
    ...
    expect(env.VINCU_DESKTOP_MANAGED).toBeUndefined();
    expect(env[VINCU_NODE_ENV]).toBeUndefined();
    expect(env.VINCU_SUPERVISED).toBeUndefined();
  });
  ...
  test("builds self node command with Electron node mode", () => {
    ...
    expect(command.env.VINCU_DESKTOP_MANAGED).toBeUndefined();
    expect(command.env[VINCU_NODE_ENV]).toBeUndefined();
    expect(command.env.VINCU_SUPERVISED).toBeUndefined();
  });
  ...
  test("does not use user NODE_ENV as Vincu runtime mode", () => {
    expect(resolveVincuNodeEnv({ NODE_ENV: "development" })).toBeUndefined();
    expect(resolveVincuNodeEnv({ NODE_ENV: "development", VINCU_NODE_ENV: "production" })).toBe(
      "production",
    );
    expect(resolveVincuNodeEnv({ NODE_ENV: "test", VINCU_NODE_ENV: "local" })).toBeUndefined();
  });
});
