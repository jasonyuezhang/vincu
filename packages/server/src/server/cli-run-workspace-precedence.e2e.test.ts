...
// The daemon-level workspace contract that `vincu run` depends on: each
...
// precedence (--workspace > $VINCU_WORKSPACE_ID > --worktree > bare) is covered
...
test("daemon resolves human and managed CLI workspace ownership", async () => {
  ...
  const cwd = mkdtempSync(path.join(tmpdir(), "vincu-cli-run-cwd-"));
  const otherCwd = mkdtempSync(path.join(tmpdir(), "vincu-cli-run-other-cwd-"));
  ...
  try {
    ...
    // $VINCU_WORKSPACE_ID land a run) creates no new workspace record: the
