...
VINCU_HOME=~/.vincu        # Runtime state directory (agents/, etc.)
VINCU_LISTEN=127.0.0.1:6767  # Listen address (host:port or /path/to/socket)
```

`VINCU_HOME` defaults to `~/.vincu` and isolates runtime artifacts like `agents/`. `VINCU_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
VINCU_HOME=~/.vincu-blue VINCU_LISTEN=127.0.0.1:7777 npm run dev
```
