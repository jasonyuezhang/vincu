...
describe("shared tool-call display mapping", () => {
  ...
  it("builds display model for worktree setup detail", () => {
    const display = buildToolCallDisplayModel({
      ...
      detail: {
        type: "worktree_setup",
        worktreePath: "/tmp/repo/.vincu/worktrees/repo/branch",
        ...
        commands: [
          {
            index: 1,
            command: "npm install",
            cwd: "/tmp/repo/.vincu/worktrees/repo/branch",
            log: "==> [1/1] Running: npm install\n",
            status: "running",
            exitCode: null,
          },
  ...
  it("humanizes Vincu MCP tool names (Claude Code format)", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__vincu__create_agent",
      status: "running",
      error: null,
  ...
  it("humanizes Vincu MCP tool names (Codex format)", () => {
    const display = buildToolCallDisplayModel({
      name: "vincu.create_agent",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
  ...
  it("humanizes list_agents Vincu tool", () => {
    const display = buildToolCallDisplayModel({
      name: "mcp__vincu__list_agents",
      status: "running",
      error: null,
      detail: { type: "unknown", input: null, output: null },
    });
