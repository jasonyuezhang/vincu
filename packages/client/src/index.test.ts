...
import { createVincuClient } from "./index.js";
import type { VincuAgent, VincuClient, VincuProviderConfig, VincuWorkspace } from "./index.js";
...
async function connectClient(): Promise<{ client: VincuClient; ws: FakeWebSocket }> {
  vi.stubGlobal("WebSocket", FakeWebSocket);
  const client = createVincuClient({
    url: "ws://daemon.test",
  ...
  expect(hello.clientId).toEqual(expect.stringMatching(/^vincu-sdk-/));
  ws.message(
    sessionMessage({
      type: "status",
      payload: {
...
function createWorkspace(input: Partial<VincuWorkspace> = {}): VincuWorkspace {
  return {
    id: "workspace_sdk",
...
function createAgent(input: Partial<VincuAgent> = {}): VincuAgent {
  return {
    id: "agent_sdk",
    provider: "codex",
    cwd: "/repo/sdk",
...
test("createVincuClient exposes workspace list through the daemon client", async () => {
