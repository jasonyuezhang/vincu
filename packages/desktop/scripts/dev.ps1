...
$env:VINCU_DEV_ROOT = $RootDir
$env:VINCU_DEV_RUNTIME_FALLBACK_ROOT = $RootDir
$DevRuntime = node "$ScriptDir\dev-runtime.mjs" | ConvertFrom-Json
$env:VINCU_ELECTRON_FLAGS = $DevRuntime.electronFlags
$env:VINCU_ELECTRON_USER_DATA_DIR = $DevRuntime.userDataDir
Remove-Item Env:\VINCU_DEV_RUNTIME_FALLBACK_ROOT -ErrorAction SilentlyContinue
...
$env:VINCU_CORS_ORIGINS = "*"

# Fully isolate the dev instance from a production Vincu install so `npm run dev`
...
# VINCU_HOME defaults to a script-managed dev home. If you override it (to point
...
if (-not $env:VINCU_HOME) {
    $env:VINCU_HOME = "$DevStateDir\vincu-home"
    $VincuHomeManaged = $true
} else {
    $VincuHomeManaged = $false
}
New-Item -ItemType Directory -Force -Path $env:VINCU_HOME, $env:VINCU_ELECTRON_USER_DATA_DIR | Out-Null

$DevDaemonPort = if ($env:VINCU_DEV_DAEMON_PORT) { $env:VINCU_DEV_DAEMON_PORT } else { "6788" }
if (-not $env:VINCU_LISTEN) { $env:VINCU_LISTEN = "127.0.0.1:$DevDaemonPort" }
...
# (it does NOT honor the VINCU_LISTEN env var) and probing that address. Without
...
# ONLY seed the script-managed home: never rewrite a user-supplied VINCU_HOME
# (that could clobber a production config.json with the dev port + wildcard CORS).
if ($VincuHomeManaged) {
    $env:TMP_CFG_PATH = "$($env:VINCU_HOME)/config.json"
...
"@
    ...
    Write-Host "  (custom VINCU_HOME - leaving its config.json untouched)"
}

Write-Host @"
======================================================
  Vincu Desktop Dev (Windows)
======================================================
  Metro:      http://localhost:$($env:EXPO_PORT)
  Daemon:     $($env:VINCU_LISTEN) (isolated)
  VINCU_HOME: $($env:VINCU_HOME)
  userData:   $($env:VINCU_ELECTRON_USER_DATA_DIR)
======================================================
"@
...
concurrently `
    ...
    "cd `"$AppDir`" && cross-env VINCU_WEB_PLATFORM=electron npx expo start --port $($env:EXPO_PORT)" `
