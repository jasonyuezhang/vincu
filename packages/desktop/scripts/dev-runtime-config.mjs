...
export function resolveDevRuntime(env = process.env) {
  const remoteDebuggingPort = env.VINCU_ELECTRON_REMOTE_DEBUGGING_PORT
    ? parsePort(env.VINCU_ELECTRON_REMOTE_DEBUGGING_PORT, "VINCU_ELECTRON_REMOTE_DEBUGGING_PORT")
    : 0;
  return {
    electronFlags: buildElectronFlags(env.VINCU_ELECTRON_FLAGS, remoteDebuggingPort),
    userDataDir: resolveDevUserDataDir({
      devRoot: env.VINCU_DEV_ROOT,
      inheritedUserDataDir: env.VINCU_ELECTRON_USER_DATA_DIR,
      fallbackRoot: env.VINCU_DEV_RUNTIME_FALLBACK_ROOT,
    }),
  };
}
