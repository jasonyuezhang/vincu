...
import { resolveVincuHome, spawnProcess } from "@getvincu/server";
import {
...
function getVincuHome(): string {
  return resolveVincuHome(process.env);
}

function logFilePath(): string {
  return path.join(getVincuHome(), DAEMON_LOG_FILENAME);
}

export function isDesktopManagedDaemonRunningSync(): boolean {
  try {
    const raw = readFileSync(path.join(getVincuHome(), "vincu.pid"), "utf-8");
