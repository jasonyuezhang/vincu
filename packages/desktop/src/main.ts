...
import {
  ...
  getVincuBrowserIdForWebContents,
  getVincuBrowserWebContentsForHostWindow,
  getVincuBrowserWebviewRegistry,
  listRegisteredVincuBrowserIds,
  isVincuBrowserWebviewAttach,
  prepareVincuBrowserWebContents,
  PendingBrowserWindowOpenRequests,
  registerBrowserWebviewNavigationGuards,
  unregisterVincuBrowserFromHost,
  registerAttachedVincuBrowser,
  setWorkspaceActiveVincuBrowserId,
  unregisterVincuBrowserHost,
} from "./features/browser-webviews/index.js";
import {
  clearVincuBrowserProfile,
  getLegacyVincuBrowserProfileSession,
  VINCU_BROWSER_PROFILE_PARTITION,
  getVincuBrowserProfileSession,
...
} from "@getvincu/protocol/agent-deep-link";
import { AgentNavigationInbox, parseAgentDeepLinkFromArgv } from "./agent-navigation.js";

const DEV_SERVER_URL = process.env.EXPO_DEV_URL ?? "http://localhost:8081";
const APP_SCHEME = "vincu";
const VINCU_DEBUG = process.env.VINCU_DEBUG === "1";
const DISABLE_SINGLE_INSTANCE_LOCK = process.env.VINCU_DISABLE_SINGLE_INSTANCE_LOCK === "1";
const APP_NAME = process.env.VINCU_TEST_APP_NAME?.trim() || "Vincu";
...
const browserKeyboard = new BrowserKeyboard(getVincuBrowserWebviewRegistry());
          ...
          { type: "separator" as const },
          {
            ...
            click: () => {
              log.info("[browser-devtools] inspect-element.request", {
                ...
                browserId: getVincuBrowserIdForWebContents(contents),
                x: params.x,
                y: params.y,
...
function getBrowserPopupWindowOptions(
  ...
  return {
    ...
    webPreferences: {
      partition: VINCU_BROWSER_PROFILE_PARTITION,
...
function installBrowserWindowOpenHandler(input: {
  ...
  contents.setWindowOpenHandler(({ url, disposition, frameName, features, postBody }) => {
    ...
    const sourceBrowserId = getVincuBrowserIdForWebContents(sourceContents);
    if (sourceBrowserId) {
...
const forcedUserDataDir = process.env.VINCU_ELECTRON_USER_DATA_DIR?.trim();
if (forcedUserDataDir) {
  ...
  try {
    ...
    // Main checkout (e.g. "vincu") gets default userData — only worktrees diverge.
    ...
    if (isWorktree) {
      app.setPath("userData", path.join(app.getPath("appData"), `Vincu-${devWorktreeName}`));
...
// Allow users to pass Chromium flags via VINCU_ELECTRON_FLAGS for debugging
// rendering issues (e.g. "--disable-gpu --ozone-platform=x11").
// Must run before app.whenReady().
const electronFlags = process.env.VINCU_ELECTRON_FLAGS?.trim();
if (electronFlags) {
...
ipcMain.handle("vincu:get-pending-open-project", (event) => {
...
ipcMain.handle("vincu:agent-navigation:ready", (event) => {
...
ipcMain.handle("vincu:browser:register-attached", (event, rawInput: unknown) => {
  const input = readAttachedBrowserInput(rawInput);
  if (!input) {
    throw new Error("Invalid attached browser registration");
  }
  const registered = registerAttachedVincuBrowser({
    ...input,
    sender: event.sender,
    profileSession: getVincuBrowserProfileSession(session),
  ...
  log.info("[browser-webview] registered", {
    ...
    registeredBrowserIds: listRegisteredVincuBrowserIds(),
  });
...
ipcMain.handle("vincu:browser:unregister-workspace-browser", async (event, browserId: unknown) => {
  if (typeof browserId === "string" && browserId.trim().length > 0) {
    const normalizedBrowserId = browserId.trim();
    const hasOtherHost = getVincuBrowserWebviewRegistry().hasBrowserInOtherHostWindow(
      event.sender.id,
      normalizedBrowserId,
    );
    unregisterVincuBrowserFromHost(event.sender.id, normalizedBrowserId);
    // COMPAT(browserProfile): added in v0.1.108; remove after 2027-01-15.
    const legacyProfile = hasOtherHost
      ? null
      : getLegacyVincuBrowserProfileSession(session, normalizedBrowserId);
    if (legacyProfile) {
      try {
...
ipcMain.handle("vincu:browser:set-workspace-active-browser", (event, rawInput: unknown) => {
  const input = readActiveBrowserInput(rawInput);
  if (input) {
    setWorkspaceActiveVincuBrowserId({ ...input, hostWebContentsId: event.sender.id });
  }
});

ipcMain.handle("vincu:browser:focus", (event, browserId: unknown): boolean => {
  if (typeof browserId !== "string" || browserId.trim().length === 0) {
    return false;
  }
  const contents = getVincuBrowserWebContentsForHostWindow(browserId, event.sender.id);
  if (!contents) {
    return false;
  }
  contents.focus();
...
ipcMain.handle("vincu:browser:open-devtools", (event, browserId: unknown) => {
  if (typeof browserId !== "string" || browserId.trim().length === 0) {
    const result = {
      ok: false,
...
ipcMain.handle("vincu:browser:clear-profile", async (_event, rawLegacyBrowserIds: unknown) => {
  const profileSessions = getVincuBrowserProfileSessions(
    session,
    readLegacyVincuBrowserIds(rawLegacyBrowserIds),
  );
...
ipcMain.handle("vincu:browser:copy-element", (_event, payload: unknown): boolean => {
...
async function createWindow(
  ...
  mainWindow.on("closed", () => {
    ...
    unregisterVincuBrowserHost(webContentsId);
  ...
  mainWindow.webContents.on("will-attach-webview", (event, webPreferences, params) => {
    if (!isVincuBrowserWebviewAttach(params)) {
  ...
  mainWindow.webContents.on("did-attach-webview", (_event, contents) => {
    prepareVincuBrowserWebContents(contents);
...
function focusExistingWindowOnAgent(target: AgentDeepLinkTarget): void {
  ...
  if (deliverable) {
    mainWindow.webContents.send("vincu:event:open-agent", deliverable);
  }
}
...
function setupSingleInstanceLock(): boolean {
  if (DISABLE_SINGLE_INSTANCE_LOCK) {
    log.info("[single-instance] disabled by VINCU_DISABLE_SINGLE_INSTANCE_LOCK");
    return true;
  }
  ...
  app.on("second-instance", (_event, commandLine) => {
    ...
    // Relaunching the app (CLI `vincu [path]`, double-click, etc.) opens a new
...
async function bootstrap(): Promise<void> {
  ...
  ipcMain.handle("vincu:window:openNew", async (_event, options?: unknown) => {
    const pendingPath =
...
function showDaemonShutdownDialog(): void {
  for (const win of BrowserWindow.getAllWindows()) {
    win.webContents.send("vincu:event:quitting", {});
  }
}
