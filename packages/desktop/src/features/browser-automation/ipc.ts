...
import { BrowserAutomationExecuteRequestSchema } from "@getvincu/protocol/browser-automation/rpc-schemas";
import type {
...
import {
  ...
  listRegisteredVincuBrowserIdsForWorkspace,
  getVincuBrowserWebContentsForHostWindow,
  getWorkspaceActiveVincuBrowserIdForHostWindow,
...
function createRegistry(hostWebContentsId: number): BrowserRegistry {
  return {
    listRegisteredBrowserIds: listRegisteredVincuBrowserIds,
    listRegisteredBrowserIdsForWorkspace: listRegisteredVincuBrowserIdsForWorkspace,
    getTabContents(browserId: string): TabContents | null {
      const contents = getVincuBrowserWebContentsForHostWindow(browserId, hostWebContentsId);
      return contents ? adaptWebContents(contents) : null;
    },
    getBrowserWorkspaceId: getVincuBrowserWorkspaceId,
    getWorkspaceActiveBrowserId(workspaceId: string): string | null {
      return getWorkspaceActiveVincuBrowserIdForHostWindow(workspaceId, hostWebContentsId);
    },
  };
}
...
export function registerBrowserAutomationIpc(options?: { ipc?: IpcHandlerRegistry }): void {
  ...
  ipc.handle("vincu:browser:execute-automation-command", async (event, rawRequest: unknown) => {
