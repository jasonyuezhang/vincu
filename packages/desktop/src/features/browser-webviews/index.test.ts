...
describe("browser webview attachment", () => {
  test("accepts only allowed URLs on the shared profile partition", () => {
    ...
    expect(
      isVincuBrowserWebviewAttach({
        src: "https://example.com",
        partition: "persist:vincu-browser-tab-a",
      }),
    ).toBe(false);
    expect(
      isVincuBrowserWebviewAttach({ src: "https://example.com", partition: "persist:foreign" }),
    ).toBe(false);
  });
  ...
  test("binds explicit browser identity to the renderer that hosts the guest", () => {
    ...
    expect(getVincuBrowserIdForWebContents(guest)).toBe("browser-a");
    expect(getVincuBrowserWorkspaceId("browser-a")).toBe("workspace-a");
    unregisterVincuBrowser("browser-a");
  });
  ...
  test("rejects a guest hosted by another renderer", () => {
    ...
    expect(getVincuBrowserIdForWebContents(guest)).toBeNull();
  });
  ...
  test("concurrent windows cannot swap browser identities", () => {
    ...
    expect(getVincuBrowserIdForWebContents(firstGuest)).toBe("browser-first");
    expect(getVincuBrowserIdForWebContents(secondGuest)).toBe("browser-second");
    unregisterVincuBrowser("browser-first");
    unregisterVincuBrowser("browser-second");
  });
  ...
  test("unregisters the same browser only from its requesting host", () => {
    ...
    unregisterVincuBrowserFromHost(firstRenderer.id, "browser-shared-hosts");

    expect(getVincuBrowserIdForWebContents(firstGuest)).toBeNull();
    expect(getVincuBrowserIdForWebContents(secondGuest)).toBe("browser-shared-hosts");
    expect(getVincuBrowserWorkspaceId("browser-shared-hosts")).toBe("workspace-shared");
    unregisterVincuBrowser("browser-shared-hosts");
  });
  ...
  test("prepares throttling once and removes registration when the guest is destroyed", () => {
    ...
    prepareVincuBrowserWebContents(guest);
    registerAttachedVincuBrowser({
    ...
    expect(getVincuBrowserIdForWebContents(guest)).toBe("browser-cleanup");
