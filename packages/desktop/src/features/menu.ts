...
import { getActiveVincuBrowserWebContentsForHostWindow } from "./browser-webviews/index.js";
        ...
        { type: "separator" },
        {
          ...
          click: withBrowserWindow((win) => {
            reloadActiveBrowserOrWindow({
              win,
              getActiveBrowserContentsForHostWindow: getActiveVincuBrowserWebContentsForHostWindow,
            });
          }),
        },
        {
...
export function setupApplicationMenu(options: ApplicationMenuOptions): void {
  ...
  ipcMain.handle("vincu:menu:showContextMenu", (event, input?: ShowContextMenuInput) => {
  ...
  ipcMain.handle("vincu:menu:set-capturing-shortcut", (_event, capturing?: boolean) => {
