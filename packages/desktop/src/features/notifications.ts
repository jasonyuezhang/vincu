...
export function registerNotificationHandlers(): void {
  ipcMain.handle("vincu:notification:isSupported", () => {
    return Notification.isSupported();
  });

  ipcMain.handle("vincu:notification:send", async (event, rawInput?: NotificationInput) => {
    ...
    notification.on("click", () => {
      ...
      if (win && data && Object.keys(data).length > 0) {
        ...
        win.webContents.send("vincu:event:notification-click", payload);
      }
