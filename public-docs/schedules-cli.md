...
description: Create and manage Vincu schedules with vincu schedule.
nav: CLI
order: 27
category: Schedules
---
...
`vincu schedule` creates and manages new-agent [schedules](/docs/schedules) from your terminal, useful for headless boxes and scripts. Every run starts a fresh agent.
...
Inside a running Vincu agent, create a heartbeat for that same conversation:

```bash
vincu heartbeat create \
  --cron "*/20 * * * *" \
  --name heartbeat \
...
```bash
vincu heartbeat update <id> --cron "*/10 * * * *"
vincu heartbeat delete <id>
```

Updating a heartbeat changes only its cron cadence and optional time zone. Its target and prompt stay fixed. Heartbeat commands require `VINCU_AGENT_ID`, which Vincu sets inside agent sessions.
...
```bash
vincu schedule ls
vincu schedule inspect <id>
vincu schedule logs <id>
vincu schedule pause <id>
vincu schedule resume <id>
vincu schedule run-once <id>
vincu schedule update <id> --every 10m --max-runs 6
vincu schedule delete <id>
```
