---
title: Workspaces
description: Understand how Vincu groups working directories, agents, terminals, and browsers into workspaces.
nav: Workspaces
order: 10
category: Workspaces
---
...
Vincu is organized around workspaces, not chats.
...
The sidebar starts with projects. A project can be a git repository, a GitHub project, or any directory on a machine running the Vincu daemon.
...
In Vincu, the workspace is the stable container. The sessions are what you run inside it.
...
The workspace is the product concept; a git worktree is one way to isolate its files. More than one workspace can refer to the same managed worktree, and Vincu removes that worktree after its last workspace is archived.
...
vincu workspace create --isolation worktree --path ~/dev/my-app --base main
```
...
Creating an agent and creating a workspace are separate actions. Pass a workspace ID when you want an agent in a specific existing workspace. A bare `vincu run` from a human shell creates a new local workspace; when one agent runs it, Vincu recognizes the caller and creates a subagent in the caller's workspace.

## Worktrees

Every workspace in Vincu is backed by a working directory. When that directory is a git worktree, you get a separate branch and isolated environment for each task.
