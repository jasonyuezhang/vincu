...
description: Copyable prompts for delegating, parallelizing, reviewing, and continuing agent work with Vincu.
...
```text
Stay as the orchestrator. Use Vincu to find the available Codex 5.6 model, then
...
```text
Create three Vincu subagents in this workspace. Have one trace the request path,
...
```text
Split these two issues between two Vincu subagents. Create a separate workspace
...
```text
Use Vincu to create a heartbeat every 10 minutes. Continue this migration in
