...
description: "Vincu orchestration skills: teach coding agents to spawn, coordinate, and manage other agents using slash commands."
nav: Skills
order: 32
...
Vincu ships orchestration skills that teach coding agents how to use Vincu tools and the CLI to spawn, coordinate, and manage other agents. Skills package common workflows as slash commands, so agents know how to orchestrate without you writing the briefing and safety rails each time.
...
- **Manual:** `npx skills add getvincu/vincu`, this installs to `~/.agents/skills/` and sets up symlinks for each agent.

When the desktop app finds installed Vincu skills, it keeps the bundled skills up to date on startup. If automatic update fails, use Settings → Integrations → Update or the manual command above.

## `/vincu`, Vincu Reference

The foundational skill. Vincu reference for managing agents and workspaces. Load it when an agent needs to create agents, send them prompts, or manage workspace isolation.

Not typically invoked directly by users, it's a reference that other skills depend on.

```
/vincu show me the Vincu CLI surface for creating an agent in a worktree-isolated workspace
```

## `/vincu-handoff`, Task Handoff
...
```
/vincu-handoff hand off the auth fix to codex in a worktree-isolated workspace
/vincu-handoff hand this to claude opus for review
```

## `/vincu-loop`, Iterative Loops
...
```
/vincu-loop keep trying until the changed test file passes, max 5 iterations
/vincu-loop babysit PR 123 until checks are green, check every 2m, max-time 1h
```

## `/vincu-committee`, Committee Planning
...
```
/vincu-committee why are the websocket connections dropping under load?
/vincu-committee plan the auth system migration
```

## `/vincu-advisor`, Advisor
...
```
/vincu-advisor did I miss anything in this migration plan?
/vincu-advisor --provider claude/opus what is the UX risk in this flow?
```
