...
buildNpmPackage rec {
  ...
  installPhase = ''
    ...
    mkdir -p $out/lib/vincu
    node scripts/trace-daemon.mjs > daemon-files.txt

    while IFS= read -r path; do
      [ -z "$path" ] && continue
      mkdir -p "$out/lib/vincu/$(dirname "$path")"
      cp -a "$path" "$out/lib/vincu/$path"
    done < daemon-files.txt

    # Root package.json lets node resolve the workspace layout when the
    # CLI/server bin starts from $out.
    cp package.json $out/lib/vincu/

    # Web UI Assets
    cp -r packages/server/dist/server/web-ui $out/lib/vincu/packages/server/dist/server/

    # Create wrapper for the server entry point (for systemd / direct use)
    mkdir -p $out/bin
    # Keep Vincu's runtime mode separate from NODE_ENV, which belongs to spawned agents.
    makeWrapper ${nodejs}/bin/node $out/bin/vincu-server \
      --add-flags "$out/lib/vincu/packages/server/dist/scripts/supervisor-entrypoint.js" \
      --set VINCU_NODE_ENV production

    # Create wrapper for the CLI
    makeWrapper ${nodejs}/bin/node $out/bin/vincu \
      --add-flags "$out/lib/vincu/packages/cli/dist/index.js" \
      --set NODE_PATH "$out/lib/vincu/node_modules"
