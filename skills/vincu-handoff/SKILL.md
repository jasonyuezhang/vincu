...
Read the **vincu** skill. Before choosing a provider, read `~/.vincu/orchestration-preferences.json` unless the user explicitly named a provider in this request. Do not create the receiving agent until you have read it.
...
Don't wait by default — the user decides whether to follow along or move on. Tell them the agent ID and how to follow along (the vincu skill explains).
