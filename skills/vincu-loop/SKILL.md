...
Read the **vincu** skill. Before choosing worker or verifier providers, read `~/.vincu/orchestration-preferences.json` unless the user explicitly named providers in this request. Do not start the loop until you have read it.

Loops are a CLI primitive: `vincu loop run`. Manage with `vincu loop ls`, `vincu loop inspect <id>`, `vincu loop logs <id>`, `vincu loop stop <id>`.
