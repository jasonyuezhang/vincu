---
name: vincu
description: Vincu reference for managing workspaces, workspace scripts, agents, schedules, and heartbeats.
---

Vincu is a daemon that supervises AI coding agents on your machine. Control it through tools or a CLI.
...
**`archive_workspace`** — `{ workspaceId }`. Archives the workspace, its agents, and its terminals. Local directories remain; Vincu removes an owned worktree only after its final active workspace reference is archived.

Worktree creation and reference accounting are implementation details of `isolation: "worktree"`.

## Workspace scripts

Configured `vincu.json` scripts use the same supervised lifecycle from tools and the CLI.
...
**`start_workspace_script`** — `{ workspaceId, scriptName }`. Starts one configured script through Vincu's managed workspace-script launcher and returns its status metadata.
...
All providers shown in this document are examples, you must resolve real providers either found in the preferences or by calling Vincu provider tooling to find a valid provider.
...
```bash
vincu workspace create --isolation worktree --mode branch-off --new-branch fix-x --base main
vincu workspace create --isolation worktree --mode checkout-branch --branch existing-work
vincu workspace create --isolation worktree --mode checkout-pr --pr-number 42
vincu run --provider codex/gpt-5.4 --mode full-access --workspace <workspace-id> "<prompt>"
vincu run --provider codex/gpt-5.4 --mode full-access --new-workspace worktree --worktree-mode branch-off --new-branch fix-x --base main "<prompt>"
vincu send <agent-id> "<follow-up>"
vincu ls
vincu schedule create --cron "*/15 * * * *" "ping main build"
vincu heartbeat create --cron "*/15 * * * *" "check the build"
```

Discover with `vincu --help` and `vincu <cmd> --help`.

**If `vincu` isn't on PATH but the desktop app is installed**, the bundled CLI is at:

- macOS: `/Applications/Vincu.app/Contents/Resources/bin/vincu`
- Linux: `<install-dir>/resources/bin/vincu`
- Windows: `C:\Program Files\Vincu\resources\bin\vincu.cmd`

The desktop app's first-run hook (`installCli`) symlinks this to `~/.local/bin/vincu` (macOS/Linux) or drops a `.cmd` trampoline (Windows) and adds `~/.local/bin` to PATH via shell rc files. If that didn't take, offer to symlink it — don't do it silently.
...
|                | Default                                                         |
| -------------- | --------------------------------------------------------------- |
| Listen address | `127.0.0.1:6767` (override `VINCU_LISTEN`)                      |
| Home           | `~/.vincu` (override `VINCU_HOME`)                              |
| Daemon log     | `$VINCU_HOME/daemon.log`                                        |
| Agent state    | `$VINCU_HOME/agents/<id>.json`                                  |
| Worktrees      | `$VINCU_HOME/worktrees/` (or `worktrees.root` in `config.json`) |
| PID file       | `$VINCU_HOME/vincu.pid`                                         |
| Health         | `GET http://127.0.0.1:6767/api/health`                          |

Debug order:

1. `tail -n 200 ~/.vincu/daemon.log`.
2. `vincu daemon status` for liveness.
