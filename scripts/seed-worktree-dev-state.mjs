...
// one interpreter every Vincu checkout already depends on.

const sourceRoot = process.env.VINCU_SOURCE_CHECKOUT_PATH;
const targetRoot = process.env.VINCU_WORKTREE_PATH || process.cwd();
...
function seedVincuHome() {
  const source = process.env.VINCU_DEV_SEED_HOME || join(sourceRoot, ".dev/vincu-home");
  const target = join(targetRoot, ".dev/vincu-home");
  ...
  if (process.env.VINCU_DEV_RESET_HOME === "1") {
