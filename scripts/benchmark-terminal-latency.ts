 ...
 *
 * Boots its OWN isolated daemon subprocess (fresh mkdtemp VINCU_HOME, random
 ...
 *
 * Output: pretty table to stdout + JSON to /tmp/vincu-terminal-bench/<ts>.json
 *
...
async function bootDaemon(): Promise<BootedDaemon> {
  ...
  const vincuHome = await mkdtemp(path.join(os.tmpdir(), "vincu-bench-home-"));
  ...
  const child = spawn(tsxBin, ["scripts/supervisor-entrypoint.ts", "--dev"], {
    ...
    env: {
      ...
      VINCU_SERVER_ID: "srv_terminal_bench",
  ...
  return { child, port, vincuHome, pid: child.pid ?? -1 };
}
...
async function main(): Promise<void> {
  ...
  console.log("Booting isolated daemon (random port, fresh VINCU_HOME)...");
  ...
  try {
    ...
    console.log(`Daemon ready: pid=${daemon.pid} port=${daemon.port} home=${daemon.vincuHome}`);

    const ctor = await loadDaemonClientCtor();
    client = await connectClient(ctor, daemon.port);

    workspaceDir = await mkdtemp(path.join(os.tmpdir(), "vincu-bench-ws-"));
    ...
    const outDir = "/tmp/vincu-terminal-bench";
    ...
    if (daemon) {
      ...
      await rm(daemon.vincuHome, { recursive: true, force: true }).catch(() => undefined);
    }
    if (workspaceDir) {
