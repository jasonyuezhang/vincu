...
const appProductName = "VincuDebug";
const appScheme = "vincu";
const preferredSimulatorType = process.env.VINCU_IOS_DEVICE_TYPE || "iPhone 16 Pro";
const vincuPort = requiredEnv("VINCU_PORT");
const worktreePath = process.env.VINCU_WORKTREE_PATH || rootDir;
const worktreeName = process.env.VINCU_BRANCH_NAME || basename(worktreePath);
const worktreeHash = createHash("sha1").update(worktreePath).digest("hex").slice(0, 8);
const simulatorName =
  process.env.VINCU_IOS_SIMULATOR_NAME || `Vincu ${worktreeName} ${worktreeHash}`;
const daemonEndpoint =
  process.env.VINCU_DEV_DAEMON_ENDPOINT ||
  `localhost:${process.env.VINCU_SERVICE_DAEMON_PORT || "6768"}`;
...
async function main() {
  ...
  metro = startMetro();
  await waitForUrl(`http://127.0.0.1:${vincuPort}/.sim`);
  console.log(`iOS preview: ${process.env.VINCU_URL || `http://127.0.0.1:${vincuPort}`}/.sim`);
...
function launchApp() {
  const metroUrl = encodeURIComponent(`http://127.0.0.1:${vincuPort}`);
  run(
    "xcrun",
...
function startMetro() {
  const child = spawn("npx", ["expo", "start", "--port", vincuPort, "--localhost"], {
    cwd: appDir,
    env: {
      ...env,
      VINCU_SERVE_SIM_PREVIEW: "1",
      VINCU_SERVE_SIM_DEVICE_UDID: simulatorUdid,
      BROWSER: "none",
    },
    stdio: "inherit",
  });
...
function requiredEnv(name) {
  ...
  if (!value) throw new Error(`${name} is required; run this as a Vincu service.`);
  return value;
}
