...
- **隐私优先：** Vincu 没有遥测、追踪，也不会强制登录。

## 快速开始

Vincu 会运行一个名为 daemon 的本地服务，用来管理你的 coding agents。桌面 app、移动 app、Web app 和 CLI 等客户端都会连接到它。
...
从 [vincu.sh/download](https://vincu.sh/download) 或 [GitHub releases 页面](https://github.com/getvincu/vincu/releases)下载。打开 app 后 daemon 会自动启动，不需要再安装其他东西。
...
- [文档](https://vincu.sh/docs)
- [配置参考](https://vincu.sh/docs/configuration)
...
vincu ls                           # 列出正在运行的 agents
vincu attach abc123                # 实时流式查看输出
vincu send abc123 "also add tests" # 发送后续任务

# 在远程 daemon 上运行
vincu --host workstation.local:6767 run "run the full test suite"
```

更多内容见[完整 CLI 参考](https://vincu.sh/docs/cli)。

## Skills

Skills 会教你的 agent 使用 Vincu 来编排其他 agents。
...
- `/vincu-handoff` — 在 agents 之间交接工作。我会用它先和 Claude 规划，再交给 Codex 实现。
- `/vincu-loop` — 让 agent 按明确验收标准循环工作（也叫 Ralph loops），也可以加 verifier。
- `/vincu-advisor` — 启动单个 agent 作为 advisor，提供第二意见，但不把工作委托出去。
- `/vincu-committee` — 组建两个风格互补的 agents，让它们后退一步做根因分析并产出计划。

## 开发

Monorepo 包结构速览：

- `packages/server`：Vincu daemon（agent 进程编排、WebSocket API、MCP server）
- `packages/app`：Expo 客户端（iOS、Android、Web）
- `packages/cli`：用于 daemon 和 agent 工作流的 `vincu` CLI
- `packages/desktop`：Electron 桌面 app
- `packages/relay`：用于远程连接的 relay 包
- `packages/website`：营销站点和文档（`vincu.sh`）
...
- [getvincu/vincu-relay](https://github.com/getvincu/vincu-relay) — 官方分布式 relay，使用 Elixir 编写
- [vincu-skins](https://github.com/huangguang1999/vincu-skins) — Vincu 桌面端社区主题与零 patch 换肤工具，支持 Agent Skill
- [vincu-vscode](https://marketplace.visualstudio.com/items?itemName=hinnes.vincu-vscode) — VS Code 扩展
...
```bash
VINCU_RELAY_ENDPOINT=127.0.0.1:8080 \
VINCU_RELAY_PUBLIC_ENDPOINT=relay.example.com:443 \
VINCU_RELAY_USE_TLS=true \
vincu daemon start
```
